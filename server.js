...
function extraerNombre(texto) {
    ...
    const patrones = [
        ...
        /^([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)(?:\s+(?:quisiera|quiero|reservar|para|un|masaje|turno|por|favor))?$/i,
    ];
    
    for (const patron of patrones) {
        const match = t.match(patron);
        if (match && match[1] && match[1].length >= 2) {
            // Limpiar el nombre (solo letras)
    ...
    // Si el texto comienza con un nombre y luego continúa
    const palabras = t.split(/\s+/);
    if (palabras.length >= 1) {
        const primeraPalabra = palabras[0];
        // Verificar que parece un nombre (empieza con mayúscula o letra)
        if (/^[A-ZÁÉÍÓÚÑ][a-záéíóúñ]{1,15}$/.test(primeraPalabra)) {
            return primeraPalabra;
        }
        // También capturar si es un nombre común en minúscula pero al inicio
        if (/^[a-záéíóúñ]{2,15}$/.test(primeraPalabra) && !palabras[1]) {
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    // ANALIZAR EL TEXTO COMPLETO PARA EXTRAER INFORMACIÓN
    // Extraer nombre
    ...
    if (paisDetectado && !cd.datos.pais) {
        ...
        // Validar si el país está autorizado
        if (!paisAutorizado(paisDetectado.codigo)) {
            return `Lo sentimos, no aceptamos reservas desde ${paisDetectado.nombre} en este momento. Por favor contacta con nuestro equipo para más información.`;
        }
    }
    ...
    // Extraer teléfono (formato 53 57 15 69)
    ...
    if (tl.includes('cambiar mi nombre') || tl.includes('cambio de nombre') || tl.includes('ahora me llamo')) {
        const nuevoNombre = extraerNombre(texto.replace(/cambiar mi nombre|cambio de nombre|ahora me llamo/gi, ''));
        if (nuevoNombre) {
            cd.datos.nombre = nuevoNombre;
            return `Ok, te reconoceré como ${nuevoNombre} a partir de ahora. ¿En qué puedo ayudarte?`;
        }
    }
    
    // Detectar si quiere cambiar el país
    if (tl.includes('cambiar país') || tl.includes('soy de') || tl.includes('de ')) {
        const nuevoPais = detectarPaisConNombre(texto);
        if (nuevoPais) {
            if (!paisAutorizado(nuevoPais.codigo)) {
                return `Lo sentimos, no aceptamos reservas desde ${nuevoPais.nombre} en este momento.`;
            }
            cd.datos.pais = nuevoPais.nombre;
            cd.datos.codigoPais = nuevoPais.codigo;
            return `País actualizado a ${nuevoPais.nombre}. ¿Necesitas algo más?`;
        }
    }
    
    // Si tiene toda la información para reservar, procesar reserva
    ...
    if (!cd.datos.pais || !cd.datos.codigoPais) {
        ...
        return `${cd.datos.nombre}, ¿de qué país eres? Necesito saberlo para confirmar tu reserva.`;
    }
    
    if (!cd.datos.masaje) {
        cd.estado = 'esperando_masaje';
        let lista = `${cd.datos.nombre}, ¿qué masaje te interesa?\n\n`;
        serviciosData.forEach((s, i) => {
            lista += `${i+1}. ${s.nombre} - ${s.precio}\n`;
        });
        return lista;
    ...
    if (!cd.datos.dia) {
        ...
        return `${cd.datos.nombre}, ¿qué día prefieres? Lunes, martes, miércoles, jueves, viernes o sábado.`;
    }
    
    if (!cd.datos.hora) {
        cd.estado = 'esperando_hora';
        return `${cd.datos.nombre}, ¿a qué hora prefieres? Tenemos 12:00, 16:00 o 20:00.`;
    }
    
    if (!cd.datos.telefono) {
        cd.estado = 'esperando_telefono';
        return `${cd.datos.nombre}, por favor dime tu número de teléfono. Por ejemplo: 53 57 15 69.`;
    }
    
    // Si llegamos aquí, preguntar qué necesita
    if (tl.includes('tipos de masaje') || tl.includes('qué masajes') || tl.includes('servicios')) {
        let lista = 'Nuestros servicios disponibles:\n\n';
        serviciosData.forEach((s, i) => {
            lista += `${i+1}. ${s.nombre} - ${s.precio}\n   ${s.descripcion}\n   Beneficios: ${(s.beneficios || []).join(', ')}\n\n`;
        });
        lista += '¿Te gustaría reservar alguno?';
        return lista;
    }
    
    if (tl.includes('horario') || tl.includes('horarios')) {
        return `Los horarios disponibles son: 12:00 del mediodía, 16:00 de la tarde y 20:00 de la noche. Lunes a Sábado. ¿Te gustaría reservar?`;
    }
    ...
    if (tl.includes('reservar') || tl.includes('turno') || tl.includes('cita')) {
        ...
        if (!cd.datos.dia) {
            cd.estado = 'esperando_dia';
            return `¿Qué día prefieres para tu ${cd.datos.masaje}? Lunes a Sábado.`;
        }
        if (!cd.datos.hora) {
            cd.estado = 'esperando_hora';
            return `¿A qué hora prefieres para el ${cd.datos.dia}? Tenemos 12:00, 16:00 o 20:00.`;
        }
        if (!cd.datos.telefono) {
            cd.estado = 'esperando_telefono';
    ...
    return `Hola ${nombreCliente}, soy el asistente de Serenity Spa. ¿En qué puedo ayudarte? Puedo ayudarte a:\n\n- Reservar un turno\n- Consultar tipos de masaje\n- Ver horarios disponibles\n- Conocer precios\n\n¿Qué necesitas?`;
}
...
async function confirmarReservaInteligente(cd, ip) {
    ...
    try {
        ...
        // Limpiar datos del cliente después de reserva exitosa
