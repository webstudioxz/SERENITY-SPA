...
// ==================== MIDDLEWARE DE DETECCIÓN DE PALABRAS PROHIBIDAS ====================
app.use((req, res, next) => {
    if (req.method === 'POST' && req.body) {
        const ip = req.ip || req.socket.remoteAddress || '0.0.0.0';
        const bodyStr = JSON.stringify(req.body).toLowerCase();
        const palabraEncontrada = contienePalabraProhibida(bodyStr);
        
        if (palabraEncontrada) {
            bloquearIP(ip, `Uso de lenguaje prohibido en solicitud: "${palabraEncontrada}"`, 'PALABRA_PROHIBIDA', palabraEncontrada, true);
            return res.status(403).json({
                error: 'BLOQUEADO',
                mensaje: `Su IP ha sido bloqueada permanentemente por usar lenguaje inapropiado: "${palabraEncontrada}"`
            });
        }
    }
    next();
