...
let personalidadIA = {
    ...
    estilo: 'Hablar en español neutro, usar emojis ocasionalmente, ser amable y servicial, ofrecer siempre ayuda concreta.',
    reglas: [
...
function detectarPaisConNombre(texto) {
    ...
    const paises = [
        ...
        { nombre: 'República Dominicana', codigo: '1', claves: ['república dominicana', 'dominicana'] },
...
function extraerNombre(texto) {
    ...
    // Si el texto es corto y no tiene palabras clave, podría ser un nombre
    const palabras = t.split(/\s+/);
    if (palabras.length === 1 && palabras[0].length >= 2 && !/\b(reservar|turno|masaje|hola|buenas|precio|horario)\b/.test(t)) {
        const nombrePotencial = palabras[0];
        if (/^[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+$/.test(nombrePotencial)) {
...
Responde de manera concisa, útil y amigable. Siempre ofrece una acción concreta.`;

        // Si no hay API key, usar respuesta local
...
function generarRespuestaLocal(mensaje, nombre, codigoPais) {
    ...
    if (/\b(gracias|agradecido)\b/.test(msg)) {
        return `¡Gracias a vos ${cliente}! 😊\n\nQue tengas un excelente día. Te esperamos en Serenity Spa cuando gustes.\n\n🌟 Recordá: Lunes a Sábado, 12:00 - 16:00 - 20:00`;
    }
...
// ============================================================
// ASISTENTE DE VOZ (CORREGIDO - SIN BUCLES)
// ============================================================
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (cd.estado === 'saludo_inicial') {
        ...
        return '¡Bienvenido a Serenity Spa! Soy tu asistente virtual. ¿Me podrías decir tu nombre y de qué país eres?';
    }
    ...
    if (cd.estado === 'confirmando_pais') {
        ...
        if (paisDetectado) {
            ...
            return `Perfecto${nombre ? ', ' + nombre : ''}. ${paisDetectado.nombre} (+${paisDetectado.codigo}) registrado.\n\n📋 Estos son nuestros masajes:\n\n1️⃣ Masaje Relajante - $45 (60 min)\n2️⃣ Masaje Corporal - $65 (90 min)\n3️⃣ Masaje Facial - $40 (45 min)\n\n¿Cuál te interesa? Podés decir el número o el nombre.`;
        }
        ...
        if (cd.intentos >= 3) {
            ...
            return `No he podido detectar tu país. Usaré el código de Cuba (+53) por defecto.\n\n📋 Estos son nuestros masajes:\n\n1️⃣ Masaje Relajante - $45 (60 min)\n2️⃣ Masaje Corporal - $65 (90 min)\n3️⃣ Masaje Facial - $40 (45 min)\n\n¿Cuál te interesa?`;
        }
        
        return `Disculpa${nombre ? ', ' + nombre : ''}, no entendí bien. ¿De qué país eres? Por ejemplo: Cuba, Argentina, México, España...`;
    }
    
    // ========== ELIGIENDO MASAJE ==========
    if (cd.estado === 'eligiendo_masaje') {
        const textoLimpio = tl.replace(/[^a-záéíóúñ0-9\s]/g, '');
        
        if (textoLimpio.includes('1') || textoLimpio.includes('uno') || textoLimpio.includes('relajante')) {
            cd.datos.masaje = 'Masaje Relajante';
            cd.estado = 'eligiendo_ubicacion';
            cd.intentos = 0;
            return `¡Excelente elección, ${nombre}! El masaje relajante es ideal para liberar tensiones.\n\n¿Dónde preferís recibirlo?\n\n1️⃣ En nuestro salón\n2️⃣ A domicilio`;
        }
        if (textoLimpio.includes('2') || textoLimpio.includes('dos') || textoLimpio.includes('corporal')) {
            cd.datos.masaje = 'Masaje Corporal';
            cd.estado = 'eligiendo_ubicacion';
            cd.intentos = 0;
            return `¡Muy buena elección, ${nombre}! El masaje corporal es completo y revitalizante.\n\n¿Dónde preferís recibirlo?\n\n1️⃣ En nuestro salón\n2️⃣ A domicilio`;
        }
        if (textoLimpio.includes('3') || textoLimpio.includes('tres') || textoLimpio.includes('facial')) {
            cd.datos.masaje = 'Masaje Facial';
            cd.estado = 'eligiendo_ubicacion';
            cd.intentos = 0;
            return `¡Perfecto, ${nombre}! El masaje facial rejuvenece y relaja.\n\n¿Dónde preferís recibirlo?\n\n1️⃣ En nuestro salón\n2️⃣ A domicilio`;
        }
        ...
        if (cd.intentos >= 3) {
            ...
            cd.estado = 'eligiendo_ubicacion';
            cd.intentos = 0;
            return `Te recomiendo el Masaje Relajante, es nuestro más popular.\n\n¿En el salón o a domicilio?`;
        }
        
        return `${nombre}, decime qué masaje te interesa:\n\n1️⃣ Relajante - $45\n2️⃣ Corporal - $65\n3️⃣ Facial - $40`;
    }
    ...
    if (cd.estado === 'eligiendo_ubicacion') {
        ...
        if (textoLimpio.includes('1') || textoLimpio.includes('salon') || textoLimpio.includes('salón') || textoLimpio.includes('local')) {
            cd.datos.tipoServicio = 'salon';
            cd.datos.ubicacion = 'Salón Serenity Spa';
            cd.estado = 'eligiendo_dia';
            cd.intentos = 0;
            return `¡Perfecto! En nuestro salón.\n\n📅 ¿Qué día te viene bien? Lunes, Martes, Miércoles, Jueves, Viernes o Sábado.`;
        }
        
        if (textoLimpio.includes('2') || textoLimpio.includes('domicilio') || textoLimpio.includes('casa') || textoLimpio.includes('hogar')) {
            cd.datos.tipoServicio = 'domicilio';
            cd.estado = 'pidiendo_direccion';
            cd.intentos = 0;
            return `¡Perfecto! A domicilio.\n\n📍 Por favor, decime tu dirección completa: calle, número, ciudad.`;
        }
        ...
        if (cd.intentos >= 3) {
            ...
            cd.estado = 'eligiendo_dia';
            cd.intentos = 0;
            return `Lo programo en nuestro salón.\n\n📅 ¿Qué día te viene bien? Lunes a Sábado.`;
        }
        
        return `${nombre}, ¿dónde preferís?\n\n1️⃣ En el salón\n2️⃣ A domicilio`;
    }
    ...
    if (cd.estado === 'pidiendo_direccion') {
        ...
        cd.intentos++;
        if (cd.intentos >= 3) {
            cd.datos.ubicacion = 'A confirmar';
            cd.estado = 'eligiendo_dia';
            return `Dejamos la dirección para confirmar después.\n\n📅 ¿Qué día preferís?`;
        }
    ...
    if (cd.estado === 'eligiendo_dia') {
        ...
        if (diaEncontrado) {
            cd.datos.dia = diaEncontrado;
            cd.estado = 'eligiendo_hora';
            cd.intentos = 0;
            return `📅 ${diaEncontrado.charAt(0).toUpperCase() + diaEncontrado.slice(1)}.\n\n⏰ ¿A qué hora?\n\n• 12 del mediodía\n• 4 de la tarde\n• 8 de la noche`;
        }
        ...
        if (cd.intentos >= 3) {
            cd.datos.dia = 'lunes';
            cd.estado = 'eligiendo_hora';
            return `Te propongo el lunes.\n\n⏰ ¿A qué hora? 12, 4 o 8.`;
        }
        
        return '📅 ¿Qué día? Lunes, martes, miércoles, jueves, viernes o sábado.';
    }
    ...
    if (cd.estado === 'eligiendo_hora') {
        ...
        if (tl.includes('12') || tl.includes('doce') || tl.includes('mediodía') || tl.includes('medio dia')) {
            horaElegida = 12;
        } else if (tl.includes('4') || tl.includes('cuatro') || tl.includes('16')) {
            horaElegida = 16;
        } else if (tl.includes('8') || tl.includes('ocho') || tl.includes('20')) {
            horaElegida = 20;
        }
        
        if (horaElegida) {
            cd.datos.hora = horaElegida;
            cd.estado = 'pidiendo_telefono';
            cd.intentos = 0;
            
            const horaTexto = horaElegida === 12 ? '12 del mediodía' : horaElegida === 16 ? '4 de la tarde' : '8 de la noche';
            return `⏰ ${horaTexto}.\n\n📞 Por último, necesito tu número de teléfono (sin código de país). Solo los números.`;
        }
        ...
        if (cd.intentos >= 3) {
            cd.datos.hora = 12;
            cd.estado = 'pidiendo_telefono';
            return `Te reservo a las 12 del mediodía.\n\n📞 Por último, ¿tu número de teléfono?`;
        }
        
        return '⏰ ¿A qué hora? Decí: 12, 4 u 8.';
    }
    ...
    if (cd.estado === 'pidiendo_telefono') {
        ...
        if (cd.intentos >= 4) {
            ...
            return 'No pude registrar tu número. Por favor, intentá de nuevo más tarde o escribinos por WhatsApp.';
        }
        
        if (numeros.length > 0 && numeros.length < 7) {
            return `El número parece muy corto. Necesito al menos 7 dígitos. ¿Podés repetirlo?`;
        }
        
        return '📞 Decime tu número de teléfono, solo los números.';
    }
    
    // ========== CANCELAR ==========
    if (tl.includes('cancelar') || tl.includes('anular') || tl.includes('eliminar')) {
        if (cd.datos.telefono) {
            try {
                ...
                if (turno) {
                    ...
                    await saveTurnos(turnos);
                    voiceClients.delete(clientId);
                    return `Listo, ${nombre || 'cliente'}. Tu turno del ${turno.dia} a las ${turno.hora}:00 ha sido cancelado.`;
                }
                return `No encontré un turno activo con ese número.`;
            } catch(e) {
                return 'Error al cancelar. Intenta de nuevo.';
            }
        }
    ...
    return `${nombre ? nombre : 'Disculpa'}, ¿podrías repetir? No te entendí bien.\n\nPodés:\n📅 Reservar un turno\n⏰ Consultar horarios\n💰 Ver precios`;
}
...
async function confirmarReservaInteligente(cd, ip) {
    ...
    try {
        ...
        const diaActual = d.dia || 'lunes';
        const horaActual = d.hora || 12;
        
        if (turnos.some(t => t.telefono === d.telefono && t.dia === diaActual)) {
            return `${d.nombre}, ya tienes un turno para el ${diaActual}. Solo se permite uno por día. ¿Querés otro día?`;
        }
        ...
        if (turnos.some(t => t.dia === diaActual && t.hora === horaActual)) {
            ...
            if (alt) {
                ...
                return `Ese horario está ocupado, ${d.nombre}. Pero tengo disponible el ${alt.dia} a las ${alt.hora}:00. ¿Te sirve? Decí "sí" o "no".`;
            }
        ...
        const ubicacionTexto = d.tipoServicio === 'domicilio' ? d.ubicacion : 'Nuestro salón';
        
        return `✅ ¡RESERVA CONFIRMADA! ✅\n\n📅 Día: ${diaActual}\n⏰ Hora: ${horaTexto}\n💆 Masaje: ${d.masaje || 'Masaje Relajante'}\n📍 Lugar: ${ubicacionTexto}\n📞 Teléfono: +${d.codigoPais} ${d.telefono}\n🌍 País: ${d.pais || 'Cuba'}\n\n¡Te esperamos en Serenity Spa! 🌿\n\nSi necesitás cancelar o modificar, decí "cancelar".`;
        
    } catch(e) {
        console.error('Error al reservar:', e);
        return `Hubo un error al procesar tu reserva, ${d.nombre}. Por favor, intentá de nuevo.`;
    }
}
