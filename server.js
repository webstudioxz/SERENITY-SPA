...
// ============================================================
// TURNOS - CON VALIDACIÓN DE CÓDIGO DE PAÍS
// ============================================================
let turnosMem = [];
...
// ============================================================
// ASISTENTE DE VOZ - IA INTELIGENTE CON CONFIRMACIÓN DE PAÍS
// ============================================================
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    // ========== CORRECCIÓN DE PAÍS ==========
    if ((tl.includes('no') || tl.includes('corregir') || tl.includes('error') || tl.includes('equivocado')) && 
        cd.datos.codigoPais && cd.estado !== 'saludo_inicial') {
        ...
        return `Disculpe${nombre ? ', ' + nombre : ''}. Vamos a corregirlo. ¿Cuál es su país? Por ejemplo: Cuba, Argentina, México, España...`;
    }
    ...
    if (cd.estado === 'saludo_inicial') {
        if (nombre) {
            cd.estado = 'confirmando_pais';
            cd.intentos = 0;
            return `Es un gusto, ${nombre}. Para evitar errores al contactarle por WhatsApp, ¿me podría decir de qué país es? Por ejemplo: Cuba, Argentina, México, España...`;
        }
        
        if (/\b(reservar|turno|masaje|horario|precio)\b/.test(tl)) {
            return 'Antes de continuar, ¿me podría decir su nombre y de qué país es, por favor?';
        }
        
        if (/\b(hola|buenas?|saludos)\b/.test(tl)) {
            return 'Hola, ¿cómo está? Sea bienvenido a Serenity Spa. ¿Me podría decir su nombre y de qué país nos contacta? Así evito errores al enviarle mensajes.';
        }
        
        return 'Bienvenido a Serenity Spa. Soy su asistente virtual. Para ayudarle mejor, ¿me podría decir su nombre y de qué país es?';
    }
    ...
    if (cd.estado === 'confirmando_pais') {
        ...
        if (paisDetectado) {
            ...
            return `Perfecto${nombre ? ', ' + nombre : ''}. Código de ${paisDetectado.nombre} (+${paisDetectado.codigo}) registrado. ¿En qué le puedo ayudar? Puede reservar un turno, consultar horarios o conocer nuestros masajes.`;
        }
        ...
        if (cd.intentos >= 3) {
            ...
            cd.datos.pais = 'Cuba';
            cd.estado = 'nombre_conocido';
            return `No he podido detectar su país. Usaré el código de Cuba (+53) por defecto. Si no es correcto, por favor dígalo ahora diciendo "corregir país". ¿En qué le puedo ayudar?`;
        }
        
        return `Disculpe${nombre ? ', ' + nombre : ''}, no entendí bien. ¿Me podría decir de qué país es? Por ejemplo: Cuba, Argentina, México, España...`;
    }
    ...
    if (cd.estado === 'nombre_conocido') {
        if (!cd.datos.pais && !cd.datos.codigoPais) {
            cd.estado = 'confirmando_pais';
            cd.intentos = 0;
            return `${nombre}, antes de continuar necesito saber de qué país es para contactarle correctamente por WhatsApp. ¿De qué país es?`;
        }
        
        if (/\b(reservar|turno|cita|agendar)\b/.test(tl)) {
            cd.estado = 'eligiendo_masaje';
            return `Perfecto, ${nombre}. ¿Qué tipo de masaje le gustaría? Tenemos:\n\n✨ Relajante - 60 min - $45\n💪 Corporal - 90 min - $65\n🌸 Facial - 45 min - $40\n\n¿Cuál prefiere?`;
        }
        
        if (/\b(horario|hora|cuándo|cuando)\b/.test(tl)) {
            return `${nombre}, nuestros horarios son de lunes a sábado:\n⏰ 12:00 del mediodía\n⏰ 4:00 de la tarde\n⏰ 8:00 de la noche\n\n¿Le gustaría reservar un turno?`;
        }
        
        if (/\b(precio|costo|vale|cuánto|cuanto)\b/.test(tl)) {
            return `${nombre}, nuestros precios:\n💆 Relajante: $45\n💆 Corporal: $65\n💆 Facial: $40\n\n¿Cuál le interesa?`;
        }
        
        if (/\b(cancelar|anular|eliminar)\b/.test(tl)) {
            cd.estado = 'cancelando';
            return `${nombre}, para cancelar su turno necesito su número de teléfono. ¿Me lo podría dar?`;
        }
        
        if (/\b(gracias|agradecido)\b/.test(tl)) {
            return `Ha sido un placer atenderle, ${nombre}. Que tenga un excelente día. ¡Vuelva pronto a Serenity Spa!`;
        }
        
        if (/\b(país|pais|código|codigo|whatsapp)\b/.test(tl)) {
            return `${nombre}, su código de país registrado es +${cd.datos.codigoPais} (${cd.datos.pais}). Si desea cambiarlo, diga "corregir país".`;
        }
        
        return `${nombre}, ¿en qué puedo ayudarle?\n\n📅 Reservar un turno\n⏰ Consultar horarios\n💰 Ver precios\n💆 Conocer masajes\n🌍 Ver mi país registrado`;
    }
    ...
    if (cd.estado === 'cancelando') {
        ...
        if (tel.length >= 7) {
            try {
                ...
                if (turno) {
                    ...
                    return `Listo, ${nombre}. Su turno del ${turno.dia} a las ${turno.hora}:00 ha sido cancelado. Que tenga buen día.`;
                }
                return `No encontré un turno activo con ese número, ${nombre}. ¿Quiere intentar con otro número?`;
            } catch(e) {
        ...
        if (cd.intentos >= 3) {
            voiceClients.delete(clientId);
            return 'No pude procesar la cancelación. Intente más tarde.';
        }
        return 'Necesito su número de teléfono para cancelar. Solo dígitos, por favor.';
    }
    
    // ========== ELIGIENDO MASAJE ==========
    if (cd.estado === 'eligiendo_masaje') {
        if (tl.includes('relajante')) {
            cd.datos.masaje = 'Masaje Relajante';
            cd.estado = 'eligiendo_ubicacion';
            return `Excelente elección, ${nombre}. El masaje relajante es ideal para liberar tensiones. ¿Prefiere en nuestro salón o a domicilio?`;
        }
        if (tl.includes('corporal')) {
            cd.datos.masaje = 'Masaje Corporal';
            cd.estado = 'eligiendo_ubicacion';
            return `Muy buena elección, ${nombre}. ¿Prefiere en el salón o a domicilio?`;
        }
        if (tl.includes('facial')) {
            cd.datos.masaje = 'Masaje Facial';
            cd.estado = 'eligiendo_ubicacion';
            return `Perfecto, ${nombre}. El masaje facial rejuvenece la piel. ¿Salón o domicilio?`;
        }
        return `${nombre}, ¿cuál de los tres masajes prefiere? Relajante, corporal o facial.`;
    }
    
    // ========== ELIGIENDO UBICACIÓN ==========
    if (cd.estado === 'eligiendo_ubicacion') {
        if (tl.includes('salon') || tl.includes('salón') || tl.includes('local')) {
            cd.datos.ubicacion = 'salon';
            cd.estado = 'eligiendo_dia';
            return `En nuestro salón. ${nombre}, ¿qué día le viene bien? Lunes a sábado.`;
        }
        if (tl.includes('domicilio') || tl.includes('casa') || tl.includes('hogar')) {
            cd.datos.ubicacion = 'domicilio';
            cd.estado = 'pidiendo_direccion';
            return `A domicilio. ${nombre}, ¿me podría dar su dirección completa?`;
        }
    ...
    if (cd.estado === 'pidiendo_direccion') {
        if (texto.trim().length > 5) {
            cd.datos.direccion = texto.trim();
            cd.estado = 'eligiendo_dia';
            return `Dirección registrada, ${nombre}. ¿Qué día le viene bien? Lunes a sábado.`;
        }
        return 'Por favor, dígame su dirección completa: calle, número y ciudad.';
    }
    ...
    if (cd.estado === 'eligiendo_dia') {
        ...
        for (const dia of dias) {
            if (tl.includes(dia)) {
                cd.datos.dia = dia;
                cd.estado = 'eligiendo_hora';
                return `${dia.charAt(0).toUpperCase() + dia.slice(1)}. ¿A qué hora prefiere? 12 del mediodía, 4 de la tarde u 8 de la noche.`;
            }
        }
        return 'Disculpe, ¿qué día? Lunes, martes, miércoles, jueves, viernes o sábado.';
    }
    
    // ========== ELIGIENDO HORA ==========
    if (cd.estado === 'eligiendo_hora') {
        let hora = null;
        if (tl.includes('12') || tl.includes('doce') || tl.includes('mediodía')) hora = 12;
        else if (tl.includes('16') || tl.includes('cuatro')) hora = 16;
        else if (tl.includes('20') || tl.includes('ocho')) hora = 20;
        
        if (hora) {
            cd.datos.hora = hora;
            cd.estado = 'pidiendo_telefono';
            return `¿Me podría dar su número de teléfono para confirmar la reserva, ${nombre}?`;
        }
        return '¿A qué hora? 12 del mediodía, 4 de la tarde u 8 de la noche.';
    }
    ...
    if (cd.estado === 'pidiendo_telefono') {
        ...
        if (numeros.length >= 7) {
            cd.datos.telefono = numeros;
            return await confirmarReservaInteligente(cd, ip);
        }
        cd.intentos++;
        if (cd.intentos >= 3) {
            voiceClients.delete(clientId);
            return 'No he podido registrar su número. Por favor, intente de nuevo más tarde.';
        }
        return `${nombre}, ¿podría repetir su número? Solo dígitos, por favor.`;
    }
    
    return `${nombre ? nombre : 'Disculpe'}, ¿podría repetir? No le he entendido bien.`;
}
...
async function confirmarReservaInteligente(cd, ip) {
    ...
    try {
        ...
        if (turnos.some(t => t.telefono === d.telefono && t.dia === d.dia)) {
            return `${d.nombre}, usted ya tiene un turno reservado para el ${d.dia}. Solo se permite un masaje por día. ¿Quiere elegir otro día?`;
        }
        ...
        if (turnos.some(t => t.dia === d.dia && t.hora === d.hora)) {
            ...
            if (alt) {
                ...
                return `Ese horario está ocupado, ${d.nombre}. Pero tengo disponible el ${alt.dia} a las ${alt.hora}:00. ¿Le parece bien?`;
            }
            return `Lo lamento, ${d.nombre}. No hay disponibilidad para ese día. ¿Quiere probar con otro día?`;
        }
        ...
        return `¡Perfecto, ${d.nombre}! Su reserva ha sido confirmada:\n\n📅 ${d.dia}\n⏰ ${horaTexto}\n💆 ${d.masaje}\n📍 ${d.ubicacion === 'domicilio' ? d.direccion : 'Nuestro salón'}\n📞 +${d.codigoPais} ${d.telefono}\n🌍 ${d.pais || ''}\n\nLe contactaremos por WhatsApp al +${d.codigoPais} ${d.telefono}.\n\n¿Es correcto? Si hay algún error, diga "corregir".\n\nLe esperamos en Serenity Spa. ¡Muchas gracias!`;
        
    } catch(e) {
        return `Hubo un error al procesar su reserva, ${d.nombre}. Por favor, intente de nuevo.`;
    }
}
