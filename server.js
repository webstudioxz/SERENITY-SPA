...
async function initAllFiles() {
    ...
    if (!fsSync.existsSync(SERVICIOS_FILE)) {
        const serviciosDefault = [
            { id: generarId(), nombre: "Masaje Relajante", precio: "$45", descripcion: "Movimientos suaves para liberar el estrés.", beneficios: ["60 Minutos de relajación"], efectos: ["Relajación profunda"], videoUrl: "", imagenWeb: "", imagenWhatsApp: "", orden: 1 },
            { id: generarId(), nombre: "Masaje Corporal", precio: "$65", descripcion: "Tratamiento completo para una relajación profunda.", beneficios: ["90 Minutos"], efectos: ["Activación linfática"], videoUrl: "", imagenWeb: "", imagenWhatsApp: "", orden: 2 },
            { id: generarId(), nombre: "Masaje Facial", precio: "$40", descripcion: "Rejuvenece la piel y alivia la tensión facial.", beneficios: ["45 Minutos"], efectos: ["Estimula colágeno"], videoUrl: "", imagenWeb: "", imagenWhatsApp: "", orden: 3 }
        ];
...
app.post('/api/cancelar-por-codigo', async (req, res) => {
    ...
    try {
        ...
        if (turnoIndex === -1) {
            // Intentos fallidos de cancelación - bloquear IP
