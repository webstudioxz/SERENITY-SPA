...
// Formas textuales de decir las horas
const HORAS_TEXTO = {
...
// Este motor analiza TODO el texto del cliente y extrae toda la información
// posible en un solo paso, permitiendo que el cliente diga todo de una vez.
...
function extraerEntidades(texto) {
    ...
    // También si menciona un masaje específico + día/hora
    ...
    // --- Nombre ---
    // Patrones explícitos: "me llamo X", "soy X", "mi nombre es X"
    const patNombre = [
    ...
    for (const p of patNombre) {
        ...
        if (m) {
            // Limpiar: quitar palabras que no son nombres
            let nombre = m[1].trim();
            // Si después del nombre hay palabras como "quiero", "masaje", cortar ahí
    ...
    // --- Teléfono (secuencias de 7+ dígitos) ---
    ...
    // Buscar números sueltos que sean horas válidas
    if (!e.hora) {
    ...
    // Buscar en texto: "doce", "cuatro", "ocho"
    if (!e.hora) {
...
// Mezclar entidades extraídas con datos ya conocidos (no sobreescribir)
...
// Determinar qué datos faltan para completar la reserva
...
// Buscar la alternativa más cercana cuando un turno está ocupado
function buscarAlternativaCercana(diaDeseado, horaDeseada, turnosExistentes) {
    const idxDia = DIAS_VALIDOS.indexOf(diaDeseado);

    // 1) Mismo día, otras horas (ordenadas por cercanía a la deseada)
    ...
    // 2) Días cercanos (alternando adelante/atrás)
...
// Este motor es INTELIGENTE: analiza el mensaje completo, extrae todas las
// entidades posibles, y determina qué falta. Si el cliente dice todo de una
// vez, salta los pasos innecesarios.
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (!clientData.estado) {
        ...
        clientData.datos = {};
        clientData.nombreRecordado = null; // Persiste entre reservas
        clientData.intentosNumero = 0;
        clientData.alternativaPendiente = null;
    }

    // Extraer TODAS las entidades del mensaje actual
    const ent = extraerEntidades(texto);
    const datos = clientData.datos;

    // Si hay un nombre recordado de una reserva anterior, usarlo
    if (!datos.nombre && clientData.nombreRecordado) {
        datos.nombre = clientData.nombreRecordado;
    }

    // Mezclar entidades extraídas con lo que ya sabemos
    mergeEntidades(datos, ent);

    // Si se extrajo un nombre, recordarlo para futuras reservas
    ...
    // ========== RESPUESTA A ALTERNATIVA PENDIENTE ==========
    ...
    if (clientData.estado === 'inicial') {
        // Si no hay intención de reservar, responder a otras consultas
        if (!ent.intencion) {
        ...
        // Hay intención de reservar — verificar qué falta
        const faltantes = queFalta(datos);

        if (faltantes.length === 0) {
            // ¡Tenemos todo! Intentar confirmar
            return await intentarConfirmarReserva(datos, clientData, ip);
        }

        // Faltan cosas — pedir la primera que falte, pero el merge ya guardó lo que sí vino
    ...
    // ========== ESTADOS INTERMEDIOS ==========
    // En CADA estado, primero hacemos merge (ya hecho arriba) y luego verificamos
    // si con lo que extrajimos ya podemos saltar pasos.

    if (clientData.estado === 'esperando_nombre') {
        if (!datos.nombre) {
            // Usar todo el texto como nombre si no se extrajo con patrones
    ...
    if (clientData.estado === 'esperando_telefono') {
        ...
        // Validar el número contra el patrón del país
        if (!validarNumeroPorPais(datos.telefono, datos.paisPattern)) {
            ...
            registrarIntentoFallido(ip);
            datos.telefono = null; // Limpiar para pedir de nuevo
...
// Generar la pregunta para el siguiente dato faltante
...
// Intentar confirmar la reserva con los datos recolectados
async function intentarConfirmarReserva(datos, clientData, ip) {
    const turnos = await cargarTurnos();

    // Verificar que no tenga ya un turno ese día
    if (datos.telefono) {
    ...
    if (ocupado) {
        // BUSCAR ALTERNATIVA MÁS CERCANA AUTOMÁTICAMENTE
    ...
    // ¡Todo disponible! Crear la reserva
    const nuevoTurno = {
        id: generarId(),
    ...
    // Resetear estado pero mantener nombre recordado
...
async function startServer() {
    try {
        ...
        const server = app.listen(PORT, '0.0.0.0', () => {
            console.log('\n' + '='.repeat(50));
            console.log('  🌿 SERENITY SPA - MOTOR INTELIGENTE v3.0');
        ...
        wss.on('connection', (ws, req) => {
            ...
            ws.on('close', () => { voiceClients.delete(clientId); console.log(`🔌 Desconectado: ${ip}`); });
        });
