...
Responde de manera concisa, útil y amigable. Siempre ofrece una acción concreta.`;
        ...
        // Si no hay API key configurada, usar respuesta local
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (process.env.DEEPSEEK_API_KEY && cd.estado === 'nombre_conocido') {
        try {
            const contexto = buscarContexto(texto);
            const systemPrompt = `Eres ${personalidadIA.nombre}, asistente de Serenity Spa. Responde de manera concisa y útil. El cliente se llama ${nombre || 'cliente'}. Contexto: ${contexto.join('. ')}`;
            ...
            console.log('IA voz no disponible, usando sistema local');
        }
    }
    
    // Sistema local (tu código existente del asistente de voz)
    if ((tl.includes('no') || tl.includes('corregir') || tl.includes('error')) && cd.datos.codigoPais && cd.estado !== 'saludo_inicial') {
        cd.datos.pais = null;
    ...
    if (cd.estado === 'saludo_inicial') {
        if (nombre) {
            cd.estado = 'confirmando_pais';
            cd.intentos = 0;
            return `Es un gusto, ${nombre}. Para evitar errores al contactarle, ¿de qué país es?`;
        }
        return 'Bienvenido a Serenity Spa. ¿Me podría decir su nombre y de qué país es?';
    }
    ...
    if (cd.estado === 'confirmando_pais') {
        ...
        if (paisDetectado) {
            ...
            cd.intentos = 0;
            return `Perfecto. ${paisDetectado.nombre} (+${paisDetectado.codigo}) registrado. ¿En qué le puedo ayudar?`;
        }
        cd.intentos++;
        if (cd.intentos >= 3) {
        ...
        return `No entendí bien. ¿De qué país es? Por ejemplo: Cuba, Argentina, México...`;
    }
    
    // ... (mantené el resto de tu código de voz existente)
...
async function confirmarReservaInteligente(cd, ip) {
    ...
    try {
        ...
        if (turnos.some(t => t.telefono === d.telefono && t.dia === d.dia)) {
            return `${d.nombre}, ya tiene un turno para el ${d.dia}.`;
        }
        if (turnos.some(t => t.dia === d.dia && t.hora === d.hora)) {
            ...
            if (alt) {
                ...
                return `Ese horario está ocupado. Tengo disponible el ${alt.dia} a las ${alt.hora}:00. ¿Le parece bien?`;
            }
            return `No hay disponibilidad. ¿Quiere probar otro día?`;
        }
        ...
        return `¡Perfecto, ${d.nombre}! Reserva confirmada:\n📅 ${d.dia}\n⏰ ${d.hora}:00\n💆 ${d.masaje}\n📍 ${d.ubicacion === 'domicilio' ? d.direccion : 'Nuestro salón'}\n📞 +${d.codigoPais} ${d.telefono}\n\nLe esperamos en Serenity Spa.`;
    } catch(e) {
