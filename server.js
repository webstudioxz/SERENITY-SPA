...
function bloquearIP(ip, motivo, tipo, palabraOfensiva, permanente = true) {
    ...
    bloqueos.set(ipLimpia, datosBloqueo);
    ...
    console.log(`📊 Total de IPs bloqueadas: ${bloqueos.size}`);
    ...
    try {
        ...
        console.log(`💾 Bloqueo guardado en archivo`);
    } catch(e) {
            ...
            <div class="blocked-card">
                ...
                <div class="reason">
                    ...
                    <span style="color:#a8906e;font-size:0.75rem;">
                        ...
                        <br>
                        🔒 Bloqueo: <span style="color:#dc2626;">PERMANENTE</span>
                    </span>
                </div>
                <p style="color:#a8906e;font-size:0.85rem;margin-bottom:1rem;line-height:1.6;">
                    ...
                    <span style="font-size:0.75rem;color:#4b5563;">
                        Esta acción es irreversible. Solo el administrador puede desbloquear su IP.
                    </span>
                </p>
                <a href="/" class="btn-back"><i class="fas fa-arrow-left"></i> Volver al inicio</a>
                <div class="admin-info">
                    <i class="fas fa-shield-alt"></i> Si considera que es un error, contacte al <a href="/admin">administrador</a>
                </div>
...
async function initAllFiles() {
    ...
    // Cargar bloqueos existentes desde el archivo
    if (fsSync.existsSync(BLOQUEOS_FILE)) {
        try {
            ...
            // Mostrar IPs bloqueadas al iniciar
            for (const [ip, datos] of bloqueos) {
                console.log(`   🔒 ${ip} - ${datos.motivo}`);
            }
        } catch(e) {
...
app.use((req, res, next) => {
    ...
    if (estaBloqueado(ip)) {
        ...
        // Para peticiones AJAX/API, devolver JSON de error con status 403
...
app.get('/health', (req, res) => {
    res.json({ 
        ...
        ipsBloqueadas: Array.from(bloqueos.keys())
    });
});
...
// ==================== CHAT IA CON BLOQUEO PERMANENTE ====================
app.post('/api/chat-ia', async (req, res) => {
    ...
    if (estaBloqueado(ip)) {
        return res.status(403).json({ 
            bloqueado: true,
            permanent: true,
            mensaje: '🚫 SU IP HA SIDO BLOQUEADA PERMANENTEMENTE',
            respuesta: '🚫 SU IP HA SIDO BLOQUEADA PERMANENTEMENTE por violar los términos de servicio.\n\nNo podrá acceder a ningún servicio de Serenity Spa.\n\nSolo el administrador puede desbloquear su IP.'
        });
    }
...
app.post('/api/seguridad/desbloquear/:ip', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) return res.status(401).json({ error: 'No autorizado' });
    ...
    if (bloqueos.has(ipDesbloquear)) {
        ...
        console.log(`✅ IP ${ipDesbloquear} desbloqueada por admin`);
