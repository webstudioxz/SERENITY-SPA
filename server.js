...
app.use((req, res, next) => {
    ...
    const rutasPermitidas = ['/admin', '/admin.html', '/login', '/login.html', '/api/login', '/api/verify', '/admin-emergencia', '/api/seguridad'];
...
app.post('/api/chat-ia', async (req, res) => {
    ...
    if (sancion.bloqueado) {
        return res.json({ respuesta: sancion.mensaje, modo: 'bloqueado', ip: sancion.ip });
    }
...
// Verificar estado de una IP específica (para el admin)
...
// Verificar estado de la IP actual (para el frontend)
