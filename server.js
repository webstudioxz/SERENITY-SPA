...
function extraerNombreAvanzado(texto) {
    ...
    // Palabras que NO pueden ser nombres
    const palabrasNoNombre = [
        'hola', 'buenas', 'saludos', 'hey', 'reservar', 'turno', 'masaje', 'precio', 'horario', 
        'ubicacion', 'direccion', 'quiero', 'necesito', 'me', 'gustaria', 'puedo', 'ayuda',
        'si', 'no', 'ok', 'vale', 'gracias', 'por favor', 'cuba', 'mexico', 'argentina', 'españa'
    ];
    ...
    for (const patron of patrones) {
        ...
        if (match && match[1] && match[1].length >= 2 && match[1].length <= 20) {
            const nombreCandidato = match[1].trim();
            // Verificar que no sea una palabra no-nombre
            if (!palabrasNoNombre.includes(nombreCandidato.toLowerCase())) {
...
function extraerTelefonoAvanzado(texto) {
    ...
    // Buscar un grupo de números de 7-15 dígitos
    const match = numeros.match(/\d{7,15}/);
    if (match && match[0].length >= 7) return match[0];
...
// Servir archivos estáticos - IMPORTANTE: debe estar ANTES de las rutas API
...
app.put('/api/servicios/:id', async (req, res) => {
    ...
    try {
        ...
        // Actualizar el servicio manteniendo el ID
        serviciosData[index] = {
            ...serviciosData[index],
            ...req.body,
            id: req.params.id
...
function generarRespuestaLocalMejorada(mensaje, nombreExistente, codigoPais) {
    ...
    if (nombreExtraido && masajeExtraido && horaExtraida && diaExtraido && telefonoExtraido) {
        return `Gracias ${nombreExtraido}. Tengo toda tu informacion: ${masajeExtraido} para el ${diaExtraido} a las ${horaExtraida}:00. Tu telefono es +${spaConfig.paisPermitido} ${telefonoExtraido}. Confirmas la reserva?`;
    }
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (tieneTodaInfo && cd.estado !== 'confirmando_reserva') {
        ...
        cd.intentos = 0;
        const horaTexto = cd.datos.hora === 12 ? '12 del mediodia' : cd.datos.hora === 16 ? '4 de la tarde' : '8 de la noche';
        return `Gracias ${cd.datos.nombre}. Tengo toda tu informacion: ${cd.datos.masaje} para el ${cd.datos.dia} a las ${horaTexto}. Tu telefono es +${spaConfig.paisPermitido} ${cd.datos.telefono}. Confirmas la reserva?`;
    }
    ...
    if (cd.estado === 'confirmando_reserva') {
        if (tl.includes('si') || tl.includes('sí') || tl.includes('confirmo') || tl.includes('vale') || tl.includes('ok')) {
            ...
            cd.datos = {};
            return 'Entiendo, cancelamos la reserva. Necesitas ayuda con algo mas?';
        } else {
            cd.intentos++;
            if (cd.intentos >= 2) {
                cd.estado = 'saludo_inicial';
                cd.datos = {};
                return 'No entendi tu respuesta. Cancelamos la reserva por seguridad. Puedes empezar de nuevo cuando quieras.';
            }
            return 'No entendi si confirmas la reserva. Responde si para confirmar o no para cancelar.';
        }
    }
    ...
    if (cd.estado === 'saludo_inicial') {
        if (nombre) {
            ...
            return `Hola ${nombre}, gusto en saludarte. Que tipo de masaje te interesa? Tenemos Masaje Relajante, Masaje Corporal o Masaje Facial.`;
        }
        
        if (tl.includes('reservar') || tl.includes('turno')) {
            return 'Claro, puedo ayudarte con eso. Cual es tu nombre?';
        }
    ...
    if (cd.estado === 'recogiendo_info') {
        if (!cd.datos.masaje) {
            return `Que tipo de masaje te interesa, ${nombre}? Tenemos Masaje Relajante, Masaje Corporal o Masaje Facial.`;
        }
        
        if (!cd.datos.dia) {
            return `Que dia prefieres para tu ${cd.datos.masaje}? Atendemos de lunes a sabado.`;
        }
        
        if (!cd.datos.hora) {
            return `A que hora te gustaria el ${cd.datos.dia}? Nuestros horarios son 12 del mediodia, 4 de la tarde o 8 de la noche.`;
        }
        
        if (!cd.datos.telefono) {
            return `Para confirmar tu reserva, necesito tu numero de telefono. Solo los numeros, por favor.`;
        }
        
        return `Para completar tu reserva, ${nombre}, necesito saber ${!cd.datos.masaje ? 'que masaje quieres' : !cd.datos.dia ? 'que dia prefieres' : !cd.datos.hora ? 'a que hora' : 'tu telefono'}.`;
    }
    
    if (tl.includes('ubicacion') || tl.includes('direccion')) {
        return `Nuestro salon esta ubicado en ${spaConfig.direccionSalon}. Tambien tenemos servicio a domicilio. Necesitas ayuda con algo mas?`;
    }
    ...
    return `Hola ${nombre || 'cliente'}, en que puedo ayudarte? Puedo ayudarte a reservar un turno, consultar horarios, precios o la ubicacion de nuestro salon.`;
}
...
async function confirmarReservaInteligente(cd, ip) {
    ...
    try {
        ...
        if (turnos.some(t => t.telefono === d.telefono && t.dia === diaActual)) {
            return `${d.nombre}, ya tienes un turno para el ${diaActual}. Solo se permite uno por dia. Quieres otro dia?`;
        }
        ...
        if (turnos.some(t => t.dia === diaActual && t.hora === horaActual)) {
            ...
            if (alt) {
                ...
                return `Ese horario esta ocupado, ${d.nombre}. Pero tengo disponible el ${alt.dia} a las ${alt.hora}:00. Te sirve?`;
            }
            return `Lo siento, ${d.nombre}. No hay disponibilidad para el ${diaActual}. Probamos otro dia?`;
        }
        ...
        const horaTexto = horaActual === 12 ? '12 del mediodia' : horaActual === 16 ? '4 de la tarde' : '8 de la noche';
        const ubicacionTexto = d.tipoServicio === 'domicilio' ? d.ubicacion : spaConfig.direccionSalon;
        
        return `Reserva confirmada. Dia: ${diaActual}. Hora: ${horaTexto}. Masaje: ${d.masaje}. Lugar: ${ubicacionTexto}. Te esperamos. Si necesitas cancelar, avisa con 4 horas de anticipacion.`;
    ...
    wss.on('connection', (ws, req) => {
        ...
        ws.on('message', async (data) => {
            ...
            try {
                ...
                if (ws.readyState === 1) {
                    ws.send(JSON.stringify({ tipo: 'respuesta', texto: 'Disculpe, hubo un error. Podria repetir?' }));
                }
            }
        });
