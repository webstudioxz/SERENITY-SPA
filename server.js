...
// Función mejorada para extraer nombre sin confundir
function extraerNombre(texto) {
    const t = texto.toLowerCase().trim();
    
    // Patrones más específicos y en orden de prioridad
    const patrones = [
        ...
        /hola\s+(?:soy|me\s+llamo|mi\s+nombre\s+es)\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+(?:\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)?)/i,
        /^hola\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]{2,12})(?:\s|$)/i,
    ];
    ...
    for (const patron of patrones) {
        ...
        if (match && match[1]) {
            ...
            nombre = nombre.replace(/[^A-ZÁÉÍÓÚÑa-záéíóúñ\s]/g, '');
            const nombreLimpio = nombre.split(/\s+/)[0];
    ...
    // Si el texto empieza con un nombre corto
    const palabras = t.split(/\s+/);
    const palabrasAccion = ['reservar', 'turno', 'masaje', 'quisiera', 'quiero', 'necesito', 'hola', 'buenas', 'cancelar', 'anular', 'por', 'favor'];
    
    if (palabras.length >= 1 && palabras[0].length >= 2 && palabras[0].length <= 12) {
        const esPalabraAccion = palabrasAccion.some(acc => t.includes(acc) && acc !== palabras[0]);
        if (!esPalabraAccion) {
            const palabrasComunes = ['hola', 'buenas', 'como', 'que', 'para', 'por', 'con', 'sin', 'mas', 'menos', 'el', 'la', 'los', 'las'];
            if (!palabrasComunes.includes(palabras[0])) {
...
function extraerTelefono(texto) {
    ...
    if (numeros.length >= 7 && numeros.length <= 12) return numeros;
    return null;
}

// Función para extraer toda la información
function extraerTodaLaInformacion(texto) {
    ...
    // Detectar cancelación (esto debe ir primero)
    if (t.includes('cancelar') || t.includes('anular') || t.includes('eliminar') || 
        (t.includes('quiero') && t.includes('cancelar')) ||
        (t.includes('necesito') && t.includes('cancelar'))) {
        resultado.esCancelacion = true;
    }
    ...
    // Extraer país (sin incluir el nombre)
    const textoRestante = texto.replace(new RegExp(resultado.nombre || '', 'gi'), '');
    const paisDetectado = detectarPaisConNombre(textoRestante);
    if (paisDetectado) {
    ...
    // Extraer masaje
    const masajeExtraido = extraerMasaje(textoRestante);
    if (masajeExtraido) resultado.masaje = masajeExtraido;
    
    // Extraer día
    const diaExtraido = extraerDia(textoRestante);
    if (diaExtraido) resultado.dia = diaExtraido;
    
    // Extraer hora
    const horaExtraida = extraerHora(textoRestante);
    if (horaExtraida) resultado.hora = horaExtraida;
    
    // Extraer teléfono
    const telefonoExtraido = extraerTelefono(textoRestante);
...
async function cancelarTurnoPorVoz(telefono, nombre, cd, ip) {
    try {
        ...
        if (turnoIndex === -1) {
            return `${nombre || 'Cliente'}, no encontré ningún turno activo con ese número de teléfono. ¿Quieres intentar con otro número o prefieres contactarnos directamente?`;
        }
        
        const turno = turnos[turnoIndex];
        const turnoInfo = `${turno.dia} a las ${turno.hora}:00 - ${turno.massageType}`;
        ...
        return `${nombre || 'Cliente'}, tu turno del ${turnoInfo} ha sido CANCELADO exitosamente. Si necesitas reservar nuevamente, solo dímelo. Gracias por avisar.`;
        
    } catch(e) {
        console.error('Error al cancelar:', e);
        return `Hubo un error al cancelar tu turno, ${nombre || 'cliente'}. Por favor, intenta de nuevo o contacta con nuestro equipo.`;
    }
}
...
async function confirmarReservaInteligente(cd, ip) {
    ...
    try {
        ...
        if (turnoExistente) {
            return `${d.nombre}, ya tienes un turno reservado para el ${d.dia} a las ${turnoExistente.hora}:00. Solo se permite un masaje por día. ¿Quieres cancelar ese turno primero?`;
        }
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    // ========== PRIORIDAD 1: CANCELAR TURNO ==========
    if (infoExtraida.esCancelacion || textoLower.includes('cancelar') || textoLower.includes('anular')) {
        if (infoExtraida.telefono) {
            return await cancelarTurnoPorVoz(infoExtraida.telefono, cd.datos.nombre || infoExtraida.nombre, cd, ip);
        }
        if (cd.datos.telefono) {
            return await cancelarTurnoPorVoz(cd.datos.telefono, cd.datos.nombre, cd, ip);
        }
        cd.estado = 'cancelando';
        return `${cd.datos.nombre || 'Cliente'}, para cancelar tu turno necesito tu número de teléfono. Por favor, dímelo.`;
    }
    ...
    if (infoExtraida.nombre && !cd.datos.nombre) {
        ...
        cd.intentos = 0;
        console.log(`✅ Nombre detectado: ${cd.datos.nombre}`);
    }
    ...
    if (cd.datos.nombre && (!cd.datos.pais || !cd.datos.codigoPais)) {
        cd.estado = 'esperando_pais';
        return `${cd.datos.nombre}, ¿de qué país nos contacta?`;
    }
    
    // ========== PRIORIDAD 4: PREGUNTAS DE INFORMACIÓN ==========
    ...
    if (textoLower.includes('precio') || textoLower.includes('costo') || textoLower.includes('cuánto cuesta')) {
    ...
    if (textoLower.includes('tipos de masaje') || textoLower.includes('qué masajes') || textoLower.includes('servicios')) {
    ...
    if (textoLower.includes('reservar') || textoLower.includes('turno') || textoLower.includes('cita') || textoLower.includes('quiero')) {
        if (!cd.datos.masaje) {
            cd.estado = 'esperando_masaje';
