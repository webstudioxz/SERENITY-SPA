...
// ============================================================
// EXTRACCIÓN DE TELÉFONO (VERSIÓN SUPER ROBUSTA)
// ============================================================
function extraerTelefono(texto, codigoPaisAdmin) {
    ...
    var patronFrasePrevia = /(?:tel[eé]fono|n[uú]mero|celular|cel|m[oó]vil|whatsapp|contacto|ll[aá]mame|escribeme|mi\s+n[uú]mero\s+es|mi\s+celular\s+es)\s*[:.]?\s*(?:es|al|el)?\s*[:.]?\s*([\d\s\-\(\)\+]{6,})/i;
    var match = textoLimpio.match(patronFrasePrevia);
    if (match) {
        var numeroExtraido = match[1].replace(/[\s\-\(\)]/g, '');
        if (numeroExtraido.length >= 7) {
            return numeroExtraido;
        }
    }
    
    var patronFinal = /(?:al|al\s+|es\s+el|es\s+)(\d[\d\s]{5,14}\d)(?:\s|\.|$)/i;
    match = textoLimpio.match(patronFinal);
    if (match) {
        var numeroExtraido = match[1].replace(/\s/g, '');
        if (numeroExtraido.length >= 7) {
            return numeroExtraido;
    ...
    var patronNumerosConEspaciosOHuiones = /\b(\d[\d\s\-]{5,14}\d)\b/g;
    var matches = [...textoLimpio.matchAll(patronNumerosConEspaciosOHuiones)];
    ...
    var patronSoloDigitosAlFinal = /[\s:.](\d{7,12})\s*$/;
    match = textoLimpio.match(patronSoloDigitosAlFinal);
    if (match) {
        return match[1];
    }
    ...
    console.log('[TELEFONO] No se pudo extraer un número válido del texto:', texto);
    return null;
}
...
function generarPreguntaInteligente(cd, faltantes) {
    ...
    switch (primeraPregunta) {
        case 'nombre':
            return 'Para poder ayudarle con su reserva, necesito saber su nombre. ¿Como se llama?';
        case 'masaje':
        ...
        case 'telefono':
            return (nombre ? nombre : '') + 'Por favor, indícame tu número de teléfono para confirmar la reserva. Solo los dígitos, sin espacios. Por ejemplo, 5551234567.';
        default:
...
// ============================================================
// PROCESAR COMANDO DE VOZ (VERSIÓN MEJORADA)
// ============================================================
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (analisis.nombre && analisis.nombre !== cd.datos.nombre) cd.datos.nombre = analisis.nombre;
    if (analisis.masaje && analisis.masaje !== cd.datos.masaje) { cd.datos.masaje = analisis.masaje; cd.datos.masajeId = analisis.masajeId; }
    if (analisis.dia && analisis.dia !== cd.datos.dia) cd.datos.dia = analisis.dia;
    if (analisis.hora && analisis.hora !== cd.datos.hora) cd.datos.hora = analisis.hora;
    if (analisis.tipoServicio && analisis.tipoServicio !== cd.datos.tipoServicio) cd.datos.tipoServicio = analisis.tipoServicio;
    if (analisis.ubicacion && analisis.ubicacion !== cd.datos.ubicacion) cd.datos.ubicacion = analisis.ubicacion;
    if (analisis.telefono && analisis.telefono !== cd.datos.telefono) cd.datos.telefono = analisis.telefono;
