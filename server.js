...
function generarPreguntaFaltante(cd, campo) {
    ...
    switch (campo) {
        ...
        case 'tipoServicio':
            return nombre + '¿Dónde prefiere recibir el masaje?\n• En nuestro salón\n• A domicilio\n\nResponda "salón" o "domicilio".';
        case 'telefono':
            return nombre + 'Para confirmar la reserva, necesito su número de teléfono. ¿Cuál es? Solo los dígitos, sin código de país.';
        default:
...
// ============================================================
// PROCESAR COMANDO DE VOZ (CORREGIDO - SIN DUPLICADOS)
// ============================================================
var voiceClients = new Map();
var processedMessages = new Map(); // Para evitar duplicados
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (!cd) {
        cd = {
            datos: { tipoServicio: 'salon', codigoPais: null, nombre: null, masaje: null, dia: null, hora: null, telefono: null, ubicacion: null },
    ...
    if (analisis.nombre && !cd.datos.nombre) {
        cd.datos.nombre = analisis.nombre;
    }
    if (analisis.masaje && !cd.datos.masaje) { 
        cd.datos.masaje = analisis.masaje; 
        cd.datos.masajeId = analisis.masajeId;
    }
    if (analisis.dia && !cd.datos.dia) {
        cd.datos.dia = analisis.dia;
    }
    if (analisis.hora && !cd.datos.hora) {
        cd.datos.hora = analisis.hora;
    }
    if (analisis.tipoServicio && !cd.datos.tipoServicio) {
        cd.datos.tipoServicio = analisis.tipoServicio;
    }
    if (analisis.ubicacion && !cd.datos.ubicacion) {
        cd.datos.ubicacion = analisis.ubicacion;
    }
    if (analisis.telefono && !cd.datos.telefono) {
    ...
    if (cd.pendienteConfirmar) {
        ...
        if (/\b(si|sí|sip|dale|ok|vale|claro|por supuesto|exacto|bien|desea|confirmo|acepto|adelante)\b/.test(t)) {
            ...
            cd.pendienteConfirmar = null;
            cd.esperandoConfirmacion = false;
            return 'Entiendo. ¿Le gustaría probar con otro día u horario? Por favor, indíqueme qué día prefiere.';
        }
    }
    
    // Verificar si el cliente está confirmando una reserva
    if (cd.esperandoConfirmacion && /\b(si|sí|sip|dale|ok|vale|claro|por supuesto|exacto|bien|desea|confirmo|acepto)\b/i.test(textoLimpio)) {
        cd.esperandoConfirmacion = false;
        return await confirmarReservaAutomatica(cd, ip);
    }
    
    // Si el cliente dice "no" a la confirmación
    if (cd.esperandoConfirmacion && /\b(no|nop|nel|cancelar|nada)\b/i.test(textoLimpio)) {
        cd.esperandoConfirmacion = false;
        return 'Entiendo. ¿Le gustaría modificar algún dato de la reserva?';
    }
    ...
    if (analisis.intencion === 'reservar' || tieneDatosReserva || (cd.datos.nombre && (cd.datos.masaje || cd.datos.dia || cd.datos.hora))) {
        var faltantes = [];
...
async function confirmarReservaAutomatica(cd, ip) {
    ...
    if (!datos.nombre) {
        cd.ultimaPregunta = 'nombre';
        return 'Para continuar, necesito saber su nombre. ¿Cuál es?';
    }
    
    if (!datos.masaje) {
        cd.ultimaPregunta = 'masaje';
        return generarMenuServicios();
    }
    ...
    if (!datos.hora) {
        ...
        return (datos.nombre || '') + '¿A qué hora prefiere? Horarios: 12:00, 16:00 o 20:00.';
    }
    
    if (!datos.tipoServicio) {
        cd.ultimaPregunta = 'tipoServicio';
    ...
    if (HORAS_VALIDAS.indexOf(horaInt) === -1) {
        cd.ultimaPregunta = 'hora';
    ...
    try {
        ...
        // Limpiar datos para próxima reserva (solo los específicos de reserva)
...
// ============================================================
// WEBSOCKET PARA ASISTENTE DE VOZ (CORREGIDO)
// ============================================================
...
async function startServer() {
    ...
    wss.on('connection', function(ws, req) {
        ...
        // Solo enviar mensaje de bienvenida UNA VEZ
        ...
        // Enviar bienvenida solo después de que la conexión esté establecida
        setTimeout(function() {
