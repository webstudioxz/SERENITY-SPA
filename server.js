...
// Función mejorada para extraer nombre
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (!datos.dia) {
        return `${datos.nombre}, ¿qué día prefieres para tu ${datos.masaje}? (Lunes a Sábado)`;
    }
    
    if (!datos.hora) {
        return `${datos.nombre}, ¿a qué hora prefieres el ${datos.dia}? Tenemos 12:00, 16:00 o 20:00.`;
    }
    ...
    if (textoLower.includes('horario') || textoLower.includes('horarios')) {
        return `Horarios: 12:00, 16:00 y 20:00. Lunes a Sábado. ¿Quieres reservar, ${datos.nombre}?`;
    }
