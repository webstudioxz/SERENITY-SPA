...
function horarioToString(hora) {
    return `${hora}:00`;
}

function stringToHorario(str) {
    ...
    try {
        ...
        if (turnos.some(t => t.dia === diaLower && t.hora === hn)) {
            return res.status(409).json({ 
                ...
                alternativa: buscarAlternativa(diaLower, hn, turnos) 
            });
        }
...
async function cancelarTurnoPorVoz(telefono, nombre, cd, ip) {
    try {
        ...
        if (turnoIndex === -1) {
            const turnoPorNombre = turnos.findIndex(t => t.nombre.toLowerCase() === (nombre || '').toLowerCase());
            if (turnoPorNombre !== -1 && turnos[turnoPorNombre].telefono) {
                return await cancelarTurnoPorVoz(turnos[turnoPorNombre].telefono, nombre, cd, ip);
            }
    ...
    try {
        ...
        if (horarioOcupado) {
            ...
            if (alt) {
                ...
                return `${d.nombre}, el horario de las ${d.hora}:00 del ${d.dia} está ocupado. Tengo disponible el ${alt.dia} a las ${horaStr}. ¿Te sirve? Responde "sí" o "no".`;
            }
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (textoLower.includes('horario') || textoLower.includes('horarios')) {
        const horasStr = horariosConfig.horarios.join(', ');
        const diasStr = DIAS_VALIDOS.map(d => d.charAt(0).toUpperCase() + d.slice(1)).join(', ');
        return `Horarios: ${horasStr}. Días: ${diasStr}. ¿Quieres reservar, ${datos.nombre}?`;
    }
