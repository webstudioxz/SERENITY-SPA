...
function extraerNombreLimpio(texto) {
    ...
    // Patrón específico: "mi nombre es X" o "me llamo X" o "soy X"
    ...
    // Si el texto comienza con "hola" o "buenas", tomar la siguiente palabra como nombre potencial
    ...
    for (let i = 0; i < palabras.length; i++) {
        ...
        if ((palabra === 'hola' || palabra === 'buenas' || palabra === 'saludos') && i + 1 < palabras.length) {
            ...
            // Verificar que no sea una palabra reservada y que tenga formato de nombre
...
function extraerInfoCompleta(texto, nombreExistente) {
    ...
    // Extraer nombre - usar función mejorada
    resultado.nombre = extraerNombreLimpio(texto);
    
    // Si no se encontró nombre con el patrón, usar el existente
    ...
    // Extraer teléfono (solo números, mínimo 7 dígitos)
...
// ==================== WEBSOCKET PARA VOZ - VERSIÓN CORREGIDA ====================
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    // Actualizar solo campos que están vacíos
    ...
    if (session.esperandoConfirmacion) {
        if (tl.includes('si') || tl.includes('sí') || tl.includes('confirmo') || tl.includes('vale') || tl.includes('ok')) {
            ...
            if (resultado.includes('confirmada')) {
                ...
                session.esperandoConfirmacion = false;
                // No borrar los datos para mantener contexto
            }
            return resultado;
    ...
    // ========== SI TIENE INFORMACIÓN COMPLETA, PEDIR CONFIRMACIÓN ==========
