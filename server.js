...
function esUrlValida(s) {
    ...
    try { const url = new URL(trimmed); return url.protocol === 'http:' || url.protocol === 'https:'; }
...
// ==================== PAÍS AUTORIZADO (DESDE CONFIG) ====================
function getPaisAutorizado() {
    if (configData && configData.paisAutorizado && configData.paisAutorizado.codigo) {
        return {
            nombre: configData.paisAutorizado.nombre || 'Cuba',
            codigo: configData.paisAutorizado.codigo || '53',
            longitudMinima: parseInt(configData.paisAutorizado.longitudMinima) || 8,
            prefijoInicial: configData.paisAutorizado.prefijoInicial || '5-7'
        };
    }
    return { nombre: 'Cuba', codigo: '53', longitudMinima: 8, prefijoInicial: '5-7' };
}

// ==================== VALIDACIÓN DE NÚMERO SEGÚN PAÍS CONFIGURADO ====================
function esNumeroValido(numero) {
    const pais = getPaisAutorizado();
    const limpio = numero.replace(/\D/g, '');
    
    // Si incluye el código de país
    const codigoStr = pais.codigo.toString();
    if (limpio.startsWith(codigoStr)) {
        const sinCodigo = limpio.substring(codigoStr.length);
        return sinCodigo.length >= pais.longitudMinima && sinCodigo.length <= 12;
    }
    
    // Sin código de país
    return limpio.length >= pais.longitudMinima && limpio.length <= 12;
}
...
function formatearNumero(numero) {
    ...
    if (limpio.startsWith(codigoStr)) return limpio;
    return codigoStr + limpio;
}

// ==================== DETECCIÓN DE PAÍS ====================
function detectarPaisConNombre(texto) {
    const t = texto.toLowerCase().trim();
    const paisAutorizado = getPaisAutorizado();
    
    if (t.includes(paisAutorizado.nombre.toLowerCase())) {
        return { nombre: paisAutorizado.nombre, codigo: paisAutorizado.codigo };
    }
    ...
    for (const otro of otrosPaises) {
        if (t.includes(otro) && otro !== paisAutorizado.nombre.toLowerCase()) {
            return { nombre: 'no_permitido', codigo: '00' };
        }
    }
...
// ==================== CONFIGURACIÓN (CON PAÍS CONFIGURABLE) ====================
let configData = {
    ...
    paisAutorizado: { nombre: "Cuba", codigo: "53", longitudMinima: "8", prefijoInicial: "5-7" }
};
...
// ==================== TURNOS CON VALIDACIÓN DE NÚMERO CONFIGURABLE ====================
let turnosMem = [];
...
app.post('/turnos', async (req, res) => {
    ...
    try {
        ...
        // VALIDACIÓN ESTRICTA DEL NÚMERO
        const tel = telefono ? telefono.replace(/\D/g, '') : '';
        if (!tel || tel.length < pais.longitudMinima) {
            return res.status(400).json({ 
                error: `Número de teléfono inválido para ${pais.nombre}. Debe tener al menos ${pais.longitudMinima} dígitos.` 
            });
        }
        
        // Validar que el número sea del país autorizado
        if (!esNumeroValido(tel)) {
            return res.status(400).json({ 
                error: `Solo se aceptan números de ${pais.nombre} (+${pais.codigo}).` 
            });
        }
        
        const numeroFormateado = formatearNumero(tel);
        ...
        if (!checkRateTel(numeroFormateado)) return res.status(429).json({ error: 'Máximo 2 turnos por teléfono por día' });
        
        const turnos = await loadTurnos();
        const dl = dia.toLowerCase();
        
        if (turnos.some(t => t.telefono === numeroFormateado && t.dia === dl)) {
            return res.status(409).json({ error: 'Ya tenés un turno ese día' });
        }
        
        if (turnos.some(t => t.dia === dl && t.hora === hn)) {
            return res.status(409).json({ error: 'Ocupado', alternativa: buscarAlternativa(dl, hn, turnos) });
        }
...
app.post('/api/cancelar-turno', async (req, res) => {
    try {
        ...
        const pais = getPaisAutorizado();
        if (tel.length < pais.longitudMinima) return res.json({ error: 'Número inválido.' });
        const turnos = await loadTurnos();
        const turno = turnos.find(t => t.telefono === tel || t.telefono === formatearNumero(tel));
...
app.post('/api/enviar-whatsapp/:id', async (req, res) => {
    try {
        ...
        const cod = t.codigoPais || pais.codigo;
        const numero = t.telefono;
        ...
        res.json({ success: true, numero: numero, mensaje: msg, urlWhatsApp: `https://wa.me/${numero}?text=${encodeURIComponent(msg)}` });
    } catch(e) { res.status(500).json({ error: 'Error' }); }
});
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (cd.estado === 'saludo_inicial') {
        if (nombre) { cd.estado = 'confirmando_pais'; cd.intentos = 0; return `Es un gusto, ${nombre}. Este servicio está disponible únicamente para ${pais.nombre}. ¿Me confirma que se encuentra en ${pais.nombre}?`; }
        if (/\b(reservar|turno|masaje|horario|precio)\b/.test(tl)) return `Antes de continuar, ¿me podría decir su nombre? Este servicio solo está disponible en ${pais.nombre}.`;
        if (/\b(hola|buenas?|saludos)\b/.test(tl)) return `Hola, ¿cómo está? Sea bienvenido a Serenity Spa. Este servicio está disponible para ${pais.nombre}. ¿Me podría decir su nombre?`;
        return `Bienvenido a Serenity Spa. Nuestro servicio está disponible para ${pais.nombre}. ¿Me podría decir su nombre?`;
    }
    
    if (cd.estado === 'confirmando_pais') {
        const paisDetectado = detectarPaisConNombre(texto);
        if (paisDetectado) {
            if (paisDetectado.codigo === '00') return `Lo siento${nombre ? ', ' + nombre : ''}, este servicio está destinado únicamente para ${pais.nombre}. No puedo procesar reservas desde otro país.`;
            cd.datos.pais = pais.nombre; cd.datos.codigoPais = pais.codigo; cd.estado = 'nombre_conocido'; cd.intentos = 0;
            return `Perfecto${nombre ? ', ' + nombre : ''}. Su reserva será para ${pais.nombre} (+${pais.codigo}). ¿En qué le puedo ayudar? Puede reservar un turno, consultar horarios o conocer nuestros masajes.`;
        }
        cd.intentos++;
        if (cd.intentos >= 3) { cd.datos.codigoPais = pais.codigo; cd.datos.pais = pais.nombre; cd.estado = 'nombre_conocido'; return `Usaré el código de ${pais.nombre} (+${pais.codigo}). ¿En qué le puedo ayudar?`; }
        return `Disculpe${nombre ? ', ' + nombre : ''}, ¿podría confirmar que se encuentra en ${pais.nombre}? Este servicio solo está disponible para residentes en ${pais.nombre}.`;
    }
    
    if (cd.estado === 'nombre_conocido') {
        if (!cd.datos.codigoPais) { cd.estado = 'confirmando_pais'; cd.intentos = 0; return `${nombre}, antes de continuar necesito confirmar que está en ${pais.nombre}. ¿Es correcto?`; }
        if (/\b(reservar|turno|cita|agendar)\b/.test(tl)) { cd.estado = 'eligiendo_masaje'; return `Perfecto, ${nombre}. ¿Qué tipo de masaje?\n\n1️⃣ Relajante - 60 min - $45\n2️⃣ Corporal - 90 min - $65\n3️⃣ Facial - 45 min - $40\n\nPuede decir el nombre o el número.`; }
        if (/\b(horario|hora|cuándo|cuando)\b/.test(tl)) return `${nombre}, horarios: lunes a sábado, 12 del mediodía, 4 de la tarde y 8 de la noche. ¿Quiere reservar?`;
        if (/\b(precio|costo|vale|cuánto|cuanto)\b/.test(tl)) return `${nombre}, Relajante: $45, Corporal: $65, Facial: $40. ¿Cuál le interesa?`;
        if (/\b(cancelar|anular|eliminar)\b/.test(tl)) { cd.estado = 'cancelando'; return `${nombre}, para cancelar necesito su número de ${pais.nombre}. ¿Me lo podría dar?`; }
        if (/\b(gracias|agradecido)\b/.test(tl)) return `Ha sido un placer, ${nombre}. ¡Vuelva pronto!`;
        return `${nombre}, ¿en qué puedo ayudarle?\n📅 Reservar | ⏰ Horarios | 💰 Precios | 💆 Masajes`;
    }
    
    if (cd.estado === 'cancelando') {
        const tel = texto.replace(/\D/g, '');
        if (tel.length >= pais.longitudMinima) {
            try {
                const turnos = await loadTurnos();
                const numeroFormateado = formatearNumero(tel);
                const turno = turnos.find(t => t.telefono === numeroFormateado || t.telefono === tel);
        ...
        return `Necesito su número de ${pais.nombre}. Mínimo ${pais.longitudMinima} dígitos.`;
    }
    
    if (cd.estado === 'eligiendo_masaje') {
        if (tl.includes('relajante') || tl.includes('suave') || tl.includes('1')) { cd.datos.masaje = 'Masaje Relajante'; cd.estado = 'eligiendo_ubicacion'; return `Relajante, 60 min por $45. ¿Salón o domicilio?`; }
        ...
        return `${nombre}, dígame: 1-Relajante, 2-Corporal, 3-Facial.`;
    }
    ...
    if (cd.estado === 'eligiendo_ubicacion') {
        if (tl.includes('salon') || tl.includes('salón') || tl.includes('local') || tl.includes('tienda') || tl.includes('centro') || tl.includes('allá') || tl.includes('alla') || tl.includes('prefiero el salón') || tl.includes('en el salon') || tl.includes('gustaría en el salón') || tl.includes('preferentemente en el salón')) {
            cd.datos.ubicacion = 'salon'; cd.estado = 'eligiendo_dia'; return `Perfecto, en nuestro salón. ¿Qué día? Lunes a sábado.`; }
        if (tl.includes('domicilio') || tl.includes('casa') || tl.includes('hogar') || tl.includes('domi') || tl.includes('mi casa') || tl.includes('en casa') || tl.includes('prefiero a domicilio') || tl.includes('gustaría a domicilio') || tl.includes('vengan') || tl.includes('venga') || tl.includes('aquí') || tl.includes('aqui') || tl.includes('donde estoy') || tl.includes('donde vivo')) {
            cd.datos.ubicacion = 'domicilio'; cd.estado = 'pidiendo_direccion'; return `Perfecto, a domicilio. ¿Me da su dirección completa?`; }
    ...
    if (cd.estado === 'pidiendo_direccion') {
        ...
        return 'Por favor, dígame su dirección completa.';
    }
    ...
    if (cd.estado === 'eligiendo_dia') {
        ...
        return `${nombre}, ¿qué día? Lunes a sábado.`;
    }
    ...
    if (cd.estado === 'eligiendo_hora') {
        ...
        if (hora) { cd.datos.hora = hora; cd.estado = 'pidiendo_telefono'; const ht = hora === 12 ? '12 del mediodía' : hora === 16 ? '4 de la tarde' : '8 de la noche'; return `${ht}. ¿Su número de ${pais.nombre}, ${nombre}?`; }
        return `${nombre}, horarios: 12 del mediodía, 4 de la tarde u 8 de la noche.`;
    }
    
    if (cd.estado === 'pidiendo_telefono') {
        const numeros = texto.replace(/\D/g, '');
        if (numeros.length >= pais.longitudMinima && esNumeroValido(numeros)) {
            cd.datos.telefono = formatearNumero(numeros);
        ...
        return `${nombre}, necesito un número válido de ${pais.nombre} (mínimo ${pais.longitudMinima} dígitos). ¿Podría repetirlo?`;
    }
...
async function confirmarReservaInteligente(cd, ip) {
    ...
    try {
        ...
        const nuevo = {
            ...
            telefono: d.telefono, codigoPais: pais.codigo,
