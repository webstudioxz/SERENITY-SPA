...
function extraerNombre(texto) {
    ...
    const palabrasIgnorar = [
        'hola', 'buenas', 'tardes', 'dias', 'noches', 'saludos', 'hey', 'soy', 'me', 'llamo', 'mi', 'nombre', 'es',
        'con', 'quien', 'tengo', 'el', 'placer', 'de', 'hablar', 'gusto', 'mucho', 'encantado', 'un',
        'quisiera', 'quiero', 'reservar', 'un', 'masaje', 'turno', 'para', 'por', 'favor', 'gracias',
        'cita', 'agendar', 'dia', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado',
        'hora', '12', '16', '20', 'precio', 'costo', 'horario', 'pais', 'cuba', 'argentina', 'mexico',
        'si', 'no', 'ok', 'vale', 'claro', 'perfecto', 'excelente'
    ];
    ...
    for (const patron of patrones) {
        ...
        if (match && match[1]) {
            ...
            if (nombre.length >= 2) return capitalizeFirst(nombre);
        }
    }
    
    // Patrón 2: respuesta simple como "Fran", "Juan", "Maria"
    ...
    for (let i = 0; i < palabras.length; i++) {
        ...
        // Verificar que no sea una palabra ignorada
        if (palabrasIgnorar.includes(palabra.toLowerCase())) {
            continue;
        }
        
        // Verificar que sea un nombre válido (solo letras)
        if (/^[A-ZÁÉÍÓÚÑa-záéíóúñ]{2,20}$/.test(palabra)) {
            // Lista de nombres comunes para validación
            const nombresComunes = /^(juan|pedro|maria|ana|luis|carlos|jose|javier|francisco|manuel|antonio|miguel|david|daniel|jorge|fernando|ricardo|pablo|andres|felipe|fran|luisa|jesus|rafael|eduardo|alejandro|roberto|hugo|oscar|ruben|sergio|alberto|victor|ramon|angel|raul|ivan|mario|clara|lucia|marta|rosa|elena|paula|sofia|valentina|camila|valeria|victoria|fernanda|carolina|natalia|alejandra|daniela|mariana|andrea|gabriela|adriana|beatriz|ines|irene|silvia|veronica|frank|fran|fransisco|ricky|tony|luisito|pedrito)$/i;
            
            if (nombresComunes.test(palabra)) {
                return capitalizeFirst(palabra);
            }
            
            // También aceptar si empieza con mayúscula (diferente a minúsculas)
            if (palabra[0] === palabra[0].toUpperCase() && palabra.toLowerCase() !== palabra) {
                return capitalizeFirst(palabra);
            }
        }
    }
    
    // Patrón 3: si el texto es muy corto y parece un nombre
    if (palabras.length === 1) {
        let unica = palabras[0].replace(/[.,!?;:]/g, '');
        if (/^[A-ZÁÉÍÓÚÑa-záéíóúñ]{2,15}$/.test(unica) && !palabrasIgnorar.includes(unica.toLowerCase())) {
            return capitalizeFirst(unica);
        }
    }
...
const paisesLista = [
    ...
    { nombre: 'Alemania', codigo: '49', claves: ['alemania', 'alem'] },
];

// Función mejorada para detectar país
function detectarPaisConNombre(texto) {
    if (!texto || typeof texto !== 'string') return null;
    
    const t = texto.toLowerCase().trim();
    
    // Buscar país directamente
    for (const pais of paisesLista) {
        for (const clave of pais.claves) {
            const regex = new RegExp('\\b' + clave + '\\b', 'i');
            if (regex.test(t)) {
                return { nombre: pais.nombre, codigo: pais.codigo };
            }
        }
    }
...
// Función para extraer teléfono
function extraerTelefono(texto) {
    if (!texto) return null;
    
    // Buscar patrón de números separados por espacios (ej: 53 57 15 69)
    const patronEspaciado = /(\d{2,3})\s+(\d{2,3})\s+(\d{2,3})\s+(\d{2,3})/;
    const matchEspaciado = texto.match(patronEspaciado);
    if (matchEspaciado) {
        const numeros = matchEspaciado.slice(1).join('');
...
// Función para extraer tipo de masaje
...
// Función para extraer día
function extraerDia(texto) {
    if (!texto) return null;
    
    const t = texto.toLowerCase();
    const dias = ['lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado', 'sábado'];
...
function extraerHora(texto) {
    ...
    if (t.includes('12') || t.includes('doce') || t.includes('mediodía') || t.includes('medio dia')) return 12;
    if (t.includes('16') || t.includes('cuatro') || t.includes('4') || t.includes('tarde')) return 16;
    if (t.includes('20') || t.includes('ocho') || t.includes('8') || t.includes('noche')) return 20;
...
async function confirmarReservaInteligente(cd, ip) {
    ...
    if (!paisAutorizado(d.codigoPais)) {
        return `Lo sentimos, no aceptamos reservas desde ${d.pais || 'su país'} en este momento.`;
    }
    ...
    try {
        ...
        if (turnos.some(t => t.telefono === d.telefono && t.dia === d.dia)) {
            return `${d.nombre}, ya tienes un turno para el ${d.dia}. Solo se permite uno por día. ¿Quieres otro día?`;
        }
        ...
        const nuevo = {
            ...
            ubicacion: d.tipoServicio === 'domicilio' ? (d.ubicacion || 'A confirmar') : paisesConfig.ubicacionSalon,
        ...
        const ubicacionTexto = d.tipoServicio === 'domicilio' ? d.ubicacion || 'A confirmar' : paisesConfig.ubicacionSalon;
...
async function procesarComandoVoz(texto, clientId, ip) {
    if (!texto || typeof texto !== 'string') {
        return 'Disculpa, no te entendí. ¿Podrías repetir?';
    }
    ...
    // Limpiar el texto de frases que no son el nombre
    let textoLimpio = texto;
    const frasesBorrar = [
        /^con quién tengo el placer de hablar\s*/i,
        /^con quién\s*/i,
        /^quién habla\s*/i,
        /^dime tu nombre\s*/i,
        /^cuál es tu nombre\s*/i,
        /^tu nombre\s*/i,
    ];
    
    for (const frase of frasesBorrar) {
        textoLimpio = textoLimpio.replace(frase, '');
    }
    
    // EXTRAER INFORMACIÓN DEL TEXTO COMPLETO
    let nombreExtraido = extraerNombre(textoLimpio);
    if (!nombreExtraido) {
        nombreExtraido = extraerNombre(texto);
    }
    
    // Si el texto es solo una palabra y parece un nombre
    if (!nombreExtraido) {
        const palabrasTexto = texto.trim().split(/\s+/);
        if (palabrasTexto.length === 1) {
            const posibleNombre = palabrasTexto[0].replace(/[.,!?]/g, '');
            if (/^[A-ZÁÉÍÓÚÑa-záéíóúñ]{2,12}$/.test(posibleNombre)) {
                const palabrasIgnorar = ['hola', 'buenas', 'si', 'no', 'ok', 'gracias'];
                if (!palabrasIgnorar.includes(posibleNombre.toLowerCase())) {
                    nombreExtraido = capitalizeFirst(posibleNombre);
                }
            }
        }
    }
    ...
    if (nombreExtraido && !cd.datos.nombre) {
        ...
        cd.intentosNombre = 0;
        console.log(`[VOZ] Nombre extraído: ${nombreExtraido} del texto: "${texto}"`);
    }
    
    // Si estamos esperando nombre y no se ha extraído
    if (cd.estado === 'esperando_nombre' && !nombreExtraido) {
        cd.intentosNombre++;
        if (cd.intentosNombre >= 2) {
            cd.datos.nombre = 'Cliente';
            cd.estado = 'esperando_pais';
            return `No entendí tu nombre. Te llamaré Cliente por ahora. ¿De qué país eres? Por ejemplo: Cuba, Argentina, México.`;
        }
        return `Disculpa, no entendí tu nombre. ¿Podrías decirlo claramente? Por ejemplo: "Me llamo Fran" o simplemente "Fran".`;
    }
    
    // Extraer país
    const paisDetectado = detectarPaisConNombre(texto);
    if (paisDetectado && !cd.datos.pais) {
        cd.datos.pais = paisDetectado.nombre;
        cd.datos.codigoPais = paisDetectado.codigo;
        console.log(`[VOZ] País extraído: ${paisDetectado.nombre} (+${paisDetectado.codigo})`);
    }
    ...
    if (masajeExtraido && !cd.datos.masaje) {
        ...
        console.log(`[VOZ] Masaje extraído: ${masajeExtraido}`);
    }
    ...
    if (diaExtraido && !cd.datos.dia) {
        ...
        console.log(`[VOZ] Día extraído: ${diaExtraido}`);
    }
    ...
    if (horaExtraida && !cd.datos.hora) {
        ...
        console.log(`[VOZ] Hora extraída: ${horaExtraida}`);
    }
    ...
    if (telefonoExtraido && !cd.datos.telefono) {
        ...
        console.log(`[VOZ] Teléfono extraído: ${telefonoExtraido}`);
    }
    
    // Verificar si ya tiene todos los datos para reservar
    ...
    // MÁQUINA DE ESTADOS
    if (cd.estado === 'saludo_inicial' || cd.estado === 'esperando_nombre') {
        cd.estado = 'esperando_nombre';
        
        if (cd.datos.nombre) {
            cd.estado = 'esperando_pais';
            return `Es un gusto, ${cd.datos.nombre}. ¿De qué país eres? Necesito el código para contactarte. Por ejemplo: Cuba, Argentina, México.`;
        }
        
        return `Hola, se ha comunicado con el asistente de Serenity Spa. ¿Con quién tengo el placer de hablar?`;
    }
    
    if (cd.estado === 'esperando_pais') {
        if (cd.datos.pais && cd.datos.codigoPais) {
            if (!paisAutorizado(cd.datos.codigoPais)) {
                return `Lo sentimos, no aceptamos reservas desde ${cd.datos.pais} en este momento. ¿Tienes otro país?`;
            }
            cd.estado = 'esperando_masaje';
            return `Perfecto, ${cd.datos.nombre}. Eres de ${cd.datos.pais}.\n\nAhora, ¿qué masaje te interesa? Tenemos:\n\n1. Masaje Relajante\n2. Masaje Corporal\n3. Masaje Facial\n\nResponde con el número o el nombre.`;
        }
        ...
        if (cd.intentos >= 2) {
            ...
            cd.datos.pais = 'Cuba';
            cd.estado = 'esperando_masaje';
            return `No pude detectar tu país. Usaré Cuba (+53) por defecto.\n\n${cd.datos.nombre}, ¿qué masaje te interesa?`;
        }
        
        return `¿De qué país eres, ${cd.datos.nombre}? Por favor, dímelo claramente.`;
    }
    
    if (cd.estado === 'esperando_masaje') {
        if (cd.datos.masaje) {
            cd.estado = 'esperando_ubicacion';
            return `Excelente elección, ${cd.datos.nombre}. El ${cd.datos.masaje} es una gran opción.\n\n¿Dónde prefieres recibirlo?\n1. En el salón\n2. A domicilio`;
        }
        
        let menu = `${cd.datos.nombre}, dime qué masaje te interesa:\n\n`;
        serviciosData.forEach((s, i) => {
    ...
    if (cd.estado === 'esperando_ubicacion') {
        const respuesta = tl.toLowerCase();
        
        if (respuesta.includes('1') || respuesta.includes('salon') || respuesta.includes('salón') || respuesta.includes('local')) {
            cd.datos.tipoServicio = 'salon';
            cd.datos.ubicacion = paisesConfig.ubicacionSalon || 'Salón Serenity Spa';
            cd.estado = 'esperando_dia';
            return `Perfecto, en nuestro salón.\n\n${cd.datos.nombre}, ¿qué día prefieres? Lunes, martes, miércoles, jueves, viernes o sábado.`;
        }
        
        if (respuesta.includes('2') || respuesta.includes('domicilio') || respuesta.includes('casa') || respuesta.includes('hogar')) {
            cd.datos.tipoServicio = 'domicilio';
            cd.estado = 'esperando_direccion';
            return `Servicio a domicilio. ${cd.datos.nombre}, por favor, dime tu dirección completa.`;
        }
        
        return `Responde 1 para salón o 2 para domicilio.`;
    }
    
    if (cd.estado === 'esperando_direccion') {
        if (texto.trim().length > 5) {
            cd.datos.ubicacion = texto.trim();
            cd.estado = 'esperando_dia';
            return `Dirección registrada: ${texto.trim()}.\n\n${cd.datos.nombre}, ¿qué día prefieres? Lunes a Sábado.`;
        }
        return `Necesito tu dirección completa para el servicio a domicilio.`;
    }
    
    if (cd.estado === 'esperando_dia') {
        if (cd.datos.dia) {
            cd.estado = 'esperando_hora';
            return `${cd.datos.nombre}, para el ${cd.datos.dia}, ¿a qué hora prefieres?\n\n12:00 (mediodía)\n16:00 (tarde)\n20:00 (noche)`;
        }
        
        return `${cd.datos.nombre}, dime un día de la semana: lunes, martes, miércoles, jueves, viernes o sábado.`;
    }
    
    if (cd.estado === 'esperando_hora') {
        if (cd.datos.hora) {
            cd.estado = 'esperando_telefono';
            return `${cd.datos.nombre}, por último, necesito tu número de teléfono. Por ejemplo: 53 52 56 12 09 (solo números).`;
        }
        
        return `¿A qué hora prefieres, ${cd.datos.nombre}? Responde 12, 16 o 20.`;
    }
    
    if (cd.estado === 'esperando_telefono') {
        if (cd.datos.telefono && cd.datos.telefono.length >= 7) {
            return await confirmarReservaInteligente(cd, ip);
        }
        
        cd.intentos++;
        if (cd.intentos >= 2) {
            cd.datos.telefono = '0000000';
            return await confirmarReservaInteligente(cd, ip);
        }
        
        return `No entendí tu número. Dilo claramente, por ejemplo: 53 57 15 69.`;
    }
    
    // Preguntas comunes
    if (tl.includes('horario') || tl.includes('horarios')) {
        return `Horarios de Serenity Spa:\n- 12:00 del mediodía\n- 16:00 de la tarde\n- 20:00 de la noche\n\nDías: Lunes a Sábado. ¿Quieres reservar?`;
    }
    
    if (tl.includes('precio') || tl.includes('costo') || tl.includes('cuánto cuesta')) {
        let lista = 'Nuestros precios:\n\n';
        serviciosData.forEach(s => {
            lista += `${s.nombre}: ${s.precio}\n`;
        });
    ...
    if (tl.includes('tipos de masaje') || tl.includes('qué masajes') || tl.includes('servicios')) {
    ...
    if (tl.includes('cancelar') || tl.includes('anular')) {
        if (cd.datos.telefono) {
            ...
            if (turno) {
                ...
                return `${cd.datos.nombre}, tu turno del ${turno.dia} a las ${turno.hora}:00 ha sido cancelado.`;
            }
        }
        return `Para cancelar, necesito tu número de teléfono.`;
    }
    
    const nombreCliente = cd.datos.nombre || 'cliente';
    return `${nombreCliente}, ¿en qué puedo ayudarte? Puedo:\n\n- Reservar un turno\n- Ver horarios\n- Consultar precios\n- Mostrar tipos de masaje`;
}
...
async function start() {
    ...
    wss.on('connection', (ws, req) => {
        ...
        const cid = generarId();
        let mc = 0;
        voiceClients.set(cid, { ws, estado: 'saludo_inicial', datos: { nombre: null, pais: null, codigoPais: null, masaje: null, dia: null, hora: null, telefono: null }, intentos: 0, clientId: cid });
