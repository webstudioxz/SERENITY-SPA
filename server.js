        ...
        <body>
            <div class="blocked-card">
                ...
                <div class="reason">
                    ...
                    ${datos?.palabraOfensiva ? `<br><br><span style="color:#f87171;">📝 Palabra ofensiva detectada: "<strong style="color:#dc2626;">${escapeHtml(datos.palabraOfensiva)}</strong>"</span>` : ''}
                    <br><br>
...
// ==================== MIDDLEWARE DE BLOQUEO (TODAS LAS RUTAS EXCEPTO ADMIN) ====================
app.use((req, res, next) => {
    const ip = req.ip || req.socket.remoteAddress || '0.0.0.0';
    const ipLimpia = getIpLimpia(ip);
    
    // Rutas que siempre están permitidas (solo admin y login)
    const rutasAdmin = [
    ...
    // Verificar si la IP está bloqueada - BLOQUEO TOTAL
    if (estaBloqueado(ip)) {
        const datos = bloqueos.get(ipLimpia);
        ...
        // Para peticiones normales, mostrar página de bloqueo
...
app.post('/api/chat-ia', async (req, res) => {
    ...
    if (palabraEncontrada) {
        ...
        // Devolver respuesta con flag de bloqueo para que el frontend recargue
        return res.json({ 
            bloqueado: true,
            ...
            if (m.tipo === 'transcripcion' && m.texto) {
                ...
                if (palabraEncontrada) {
                    ...
                    ws.send(JSON.stringify({ 
                        tipo: 'respuesta', 
                        bloqueado: true,
                        texto: `🚫 SU IP HA SIDO BLOQUEADA PERMANENTEMENTE por usar lenguaje inapropiado ("${palabraEncontrada}"). La conexión se cerrará.` 
                    }));
                    ws.close();
                    return;
                }
