...
const MODELOS_GRATUITOS = [
    { id: 'google/gemini-2.0-flash-exp:free', nombre: 'Gemini 2.0 Flash' },
    { id: 'meta-llama/llama-3.2-3b-instruct:free', nombre: 'Llama 3.2' },
    { id: 'microsoft/phi-3-mini-128k-instruct:free', nombre: 'Phi-3 Mini' }
];
...
function cambiarAlSiguienteModelo() {
    ...
    console.log(`🔄 Cambiando a modelo: ${MODELOS_GRATUITOS[modeloActualIndex].id}`);
...
Asistente: "Hola, bienvenido a Serenity Spa. Para ayudarte a reservar, ¿cuál es tu nombre?"`;
    ...
    for (let intento = 0; intento < MODELOS_GRATUITOS.length * 2; intento++) {
        try {
            ...
            console.log(`🔄 Probando modelo: ${modeloActual}`);
        }
    }
    
    console.log('⚠️ Todos los modelos gratuitos fallaron');
    return null;
}

// ==================== FUNCIÓN DE RESPUESTA LOCAL (FALLBACK) ====================
function respuestaLocalFallback(mensaje, estadoSesion) {
    ...
    if (tieneTodo) {
        ...
        return `${datos.nombre}, confirmo: ${datos.masaje} el ${datos.dia} a las ${horaTexto} ${lugarTexto}. Teléfono: +53 ${datos.telefono}. ¿Confirmas la reserva? Responde sí o no.`;
    }
        ...
        try {
            ...
            if (msg.tipo === 'transcripcion') {
                ...
                if (!respuesta) {
                    ...
                    // Actualizar nombre del cliente desde el estado local
                    if (estadoSesion.datos.nombre && !nombreCliente) {
                        nombreCliente = estadoSesion.datos.nombre;
                    }
                } else {
                    // Intentar extraer nombre de la respuesta de OpenRouter
                    const nombreMatch = respuesta.match(/gracias\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)/i);
                    if (nombreMatch && !nombreCliente) {
                        nombreCliente = nombreMatch[1];
                    }
                }
