...
let palabrasBaneadas = [
    ...
    'culo', 'cojones', 'pelotudo', 'boludo', 'forro', 'mogólico', 'xxx'
];
...
async function cancelarPorCodigo(codigo, ip, telefono) {
    try {
        const turnos = await loadTurnos();
        ...
        // Registrar la cancelación para control de abuso
        registrarCancelacion(ip, telefono || turno.telefono, codigo);
...
app.post('/turnos', async (req, res) => {
    ...
    try {
        ...
        if (palabraOfensiva) {
            bloquearIP(ip, `Uso de lenguaje ofensivo en el nombre: "${nombre}" contiene "${palabraOfensiva}"`, 'Lenguaje Ofensivo', palabraOfensiva, false);
            return res.status(403).json({ error: 'Servicio suspendido por uso de lenguaje inapropiado.' });
        }
...
app.post('/api/seguridad/bloquear-permanente/:ip', (req, res) => {
    if (!checkAuth(req)) return res.status(401).json({ error: 'No autorizado' });
    bloquearIP(req.params.ip, 'Bloqueo permanente manual', 'manual', null, true);
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (cd.esperandoCodigoCancelacion) {
        ...
        if (/^[A-Z0-9]{6}$/.test(codigo)) {
            ...
            const resultado = await cancelarPorCodigo(codigo, ip, cd.datos?.telefono);
    ...
    if (textoLower.includes('cancelar') || textoLower.includes('anular') || 
        ...
        if (codigoMatch) {
            const resultado = await cancelarPorCodigo(codigoMatch[1], ip, cd.datos?.telefono);
            return resultado.mensaje;
        }
        
        cd.esperandoCodigoCancelacion = true;
        return `Para cancelar tu reserva, necesito tu código de cancelación de 6 dígitos. Por favor, ingresa tu código ahora.\n\nSi no tienes el código, revisa el mensaje de confirmación que te enviamos al reservar.`;
    }
    ...
    // Verificar si tenemos todos los datos para reservar
