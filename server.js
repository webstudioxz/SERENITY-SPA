...
if (OPENROUTER_API_KEY && OPENROUTER_API_KEY !== '') {
    try {
        ...
        usarOpenRouter = false;
    }
} else {
    console.log('⚠️ OPENROUTER_API_KEY no encontrada, usando modo local');
}
...
function extraerInfoCompleta(texto, nombreExistente) {
    ...
    if (t.includes('12') || t.includes('doce') || t.includes('mediodía')) {
        resultado.hora = 12;
    } else if (t.includes('16') || t.includes('4') || t.includes('cuatro') || t.includes('tarde')) {
        resultado.hora = 16;
    } else if (t.includes('20') || t.includes('8') || t.includes('ocho') || t.includes('noche')) {
        resultado.hora = 20;
    }
    ...
    if (t.includes('salon') || t.includes('salón')) {
        resultado.tipoServicio = 'salon';
    } else if (t.includes('domicilio') || t.includes('casa')) {
...
app.post('/api/chat-ia', async (req, res) => {
    ...
    const contexto = construirContexto(mensajeLimpio, nombre, codigoPais);
...
Responde de manera natural y profesional. No uses emojis ni asteriscos. Sé conciso y útil.`;

    try {
        // INTENTAR CON OPENROUTER PRIMERO
        if (usarOpenRouter && openrouter) {
            try {
                ...
                console.log(`🤖 OpenRouter respondió (${MODELO_IA})`);
                return res.json({ respuesta, modo: 'openrouter', modelo: MODELO_IA });
            } catch (error) {
                console.error('❌ Error con OpenRouter:', error.message);
                // Si falla OpenRouter, usar respuesta local
            }
        }
        
        // FALLBACK: Respuesta local inteligente
        const respuestaLocal = generarRespuestaLocalConIA(mensajeLimpio, nombre, codigoPais);
...
function construirContexto(mensaje, nombre, codigoPais) {
    // Obtener información relevante de los servicios
    const serviciosInfo = serviciosData.map(s => 
        `${s.nombre}: ${s.descripcion}. Precio: ${s.precio}. Beneficios: ${(s.beneficios || []).join(', ')}`
    ).join('\n');
    
    return `
Horarios de atención: ${spaConfig.horarios}
...
Cliente actual: ${nombre || 'No especificado'}
Código de país: +${codigoPais || spaConfig.paisPermitido}

Reglas del negocio:
- Solo se aceptan reservas para el país ${spaConfig.paisNombre}
- Horarios: solo 12:00, 16:00 y 20:00
- Días: lunes a sábado
- Un turno por persona por día
- Cancelación con 4 horas de anticipación
`;
}

function generarRespuestaLocalConIA(mensaje, nombreExistente, codigoPais) {
    ...
    // Verificar si el mensaje contiene una reserva completa
    if (infoExtraida.nombre && infoExtraida.masaje && infoExtraida.hora && infoExtraida.dia && infoExtraida.telefono) {
        return `Gracias ${infoExtraida.nombre}. Tengo toda tu información: ${infoExtraida.masaje} para el ${infoExtraida.dia} a las ${infoExtraida.hora}:00. Tu teléfono es +${spaConfig.paisPermitido} ${infoExtraida.telefono}. ¿Confirmas la reserva? Responde sí o no.`;
    }
    
    // Verificar si es una confirmación de reserva
    if (msg.includes('si') || msg.includes('sí') || msg.includes('confirmo')) {
        return 'Perfecto, procederé a confirmar tu reserva. ¿En el salón o a domicilio? Responde 1 para salón o 2 para domicilio.';
    }
    
    if (msg.includes('no') && (msg.includes('confirmo') || msg.includes('reserva'))) {
        return 'Entiendo, cancelamos la reserva. ¿Necesitas ayuda con algo más?';
    }
    
    // Preguntas comunes
    if (msg.includes('ubicacion') || msg.includes('dirección') || msg.includes('donde esta')) {
    ...
    // Flujo de reserva - preguntar información faltante
    ...
    const nombre = nombreExistente || infoExtraida.nombre || 'cliente';
    
    if (!infoExtraida.masaje) {
        return `${nombre}, ¿qué tipo de masaje te interesa? Tenemos: 1. Masaje Relajante, 2. Masaje Corporal, 3. Masaje Facial.`;
    }
    
    if (!infoExtraida.dia) {
        return `${nombre}, ¿qué día prefieres para tu ${infoExtraida.masaje}? Atendemos de lunes a sábado.`;
    }
    
    if (!infoExtraida.hora) {
        return `${nombre}, ¿a qué hora te gustaría el ${infoExtraida.dia}? Nuestros horarios son 12 del mediodía, 4 de la tarde o 8 de la noche.`;
    }
    
    if (!infoExtraida.telefono) {
        return `${nombre}, para confirmar tu reserva, necesito tu número de teléfono. Solo los números, por favor.`;
    }
    
    return `${nombre}, ¿en qué puedo ayudarte? Puedo ayudarte a reservar un turno, consultar horarios, precios o nuestra ubicación.`;
}
...
app.get('/health', (req, res) => {
    res.json({ 
        ...
        ia: usarOpenRouter ? 'openrouter' : 'local',
        modelo: usarOpenRouter ? MODELO_IA : 'local',
...
async function procesarComandoVoz(texto, clientId, ip) {
    ...
    if (tl.includes('cancelar') && session.reservaActiva) {
        ...
        session.esperandoTipoServicio = false;
        session.datos = {
    ...
    if (session.reservaActiva && !session.esperandoConfirmacion && !session.esperandoTipoServicio) {
        ...
        if (tl.includes('horario') || tl.includes('horarios')) {
            return `Nuestros horarios son: ${spaConfig.horarios}. Tu reserva está confirmada para el ${session.ultimaReserva?.dia} a las ${session.ultimaReserva?.hora}:00. ¿Necesitas algo más?`;
        }
        ...
        if (tl.includes('tipos') || tl.includes('masajes')) {
            let lista = 'Nuestros masajes: ';
            serviciosData.forEach((s, i) => {
                lista += `${i+1}. ${s.nombre} ${s.precio}, `;
            });
            return lista.slice(0, -2) + '. ¿Te interesa alguno para una próxima visita?';
        }
    ...
    // ========== ESPERANDO TIPO DE SERVICIO ==========
    if (session.esperandoTipoServicio) {
        if (tl.includes('1') || tl.includes('salon') || tl.includes('salón')) {
            session.datos.tipoServicio = 'salon';
            session.datos.ubicacion = spaConfig.direccionSalon;
            session.esperandoTipoServicio = false;
            ...
            return `Perfecto, servicio en el salón. Confirmo: ${session.datos.masaje} para el ${session.datos.dia} a las ${horaTexto} en ${spaConfig.direccionSalon}. Teléfono +${spaConfig.paisPermitido} ${session.datos.telefono}. ¿Confirmas la reserva? Responde sí o no.`;
        } else if (tl.includes('2') || tl.includes('domicilio') || tl.includes('casa')) {
            session.datos.tipoServicio = 'domicilio';
            return `Servicio a domicilio. Por favor, indícame tu dirección completa.`;
        } else {
            return '¿Dónde prefieres recibir el masaje? Responde 1 para el salón o 2 para domicilio.';
        }
    }
    
    // ========== ESPERANDO DIRECCIÓN PARA DOMICILIO ==========
    if (session.datos.tipoServicio === 'domicilio' && !session.datos.ubicacion) {
        if (textoOriginal.length > 5) {
            session.datos.ubicacion = textoOriginal;
            session.esperandoConfirmacion = true;
            const horaTexto = session.datos.hora === 12 ? '12 del mediodía' : session.datos.hora === 16 ? '4 de la tarde' : '8 de la noche';
            return `Dirección registrada: ${textoOriginal}. Confirmo: ${session.datos.masaje} para el ${session.datos.dia} a las ${horaTexto} en tu domicilio. Teléfono +${spaConfig.paisPermitido} ${session.datos.telefono}. ¿Confirmas la reserva? Responde sí o no.`;
        } else {
            return 'Por favor, proporciona tu dirección completa: calle, número y ciudad.';
        }
    }
    ...
    // ========== SI TIENE INFORMACIÓN COMPLETA ==========
    if (tieneInfoCompleta && !session.datos.tipoServicio) {
        session.esperandoTipoServicio = true;
        return `Gracias ${session.datos.nombre}. Tengo tu información: ${session.datos.masaje} para el ${session.datos.dia} a las ${session.datos.hora === 12 ? '12 del mediodía' : session.datos.hora === 16 ? '4 de la tarde' : '8 de la noche'}. Tu teléfono es +${spaConfig.paisPermitido} ${session.datos.telefono}. ¿Dónde prefieres recibir el masaje? Responde 1 para el salón o 2 para domicilio.`;
    }
    ...
    if (!session.datos.nombre) {
        ...
        // Intentar usar IA para respuestas más naturales
        if (usarOpenRouter && openrouter) {
            try {
                const respuestaIA = await generarRespuestaConOpenRouter(texto, null, null);
                return respuestaIA;
            } catch(e) {
    ...
    return `${session.datos.nombre}, ¿en qué puedo ayudarte? Puedo ayudarte a reservar un turno, consultar horarios, precios o nuestra ubicación.`;
}

async function generarRespuestaConOpenRouter(mensaje, nombre, codigoPais) {
    if (!usarOpenRouter || !openrouter) {
        return null;
    }
    
    try {
        const contexto = construirContexto(mensaje, nombre, codigoPais);
        ...
        const completion = await openrouter.chat.completions.create({
            ...
            messages: [
                { 
                    role: 'system', 
                    content: `Eres el asistente virtual de Serenity Spa. Debes ser profesional, cálido y conciso. No uses emojis ni asteriscos. Tu objetivo es ayudar al cliente a reservar turnos.

Información del negocio:
${contexto}

Reglas:
- Solo aceptas reservas para ${spaConfig.paisNombre} (código +${spaConfig.paisPermitido})
- Horarios: solo 12:00, 16:00, 20:00
- Días: lunes a sábado
- Un turno por persona por día`
                },
        ...
        console.error('Error generando respuesta con OpenRouter:', e.message);
        return null;
    }
}
...
async function start() {
    ...
    const server = app.listen(PORT, '0.0.0.0', () => {
        ...
        console.log(`🤖 IA: ${usarOpenRouter ? `OpenRouter (${MODELO_IA})` : 'Modo local'}`);
    ...
    wss.on('connection', (ws, req) => {
        ...
        console.log(`🎤 Cliente de voz conectado: ${ip}`);
        ...
        ws.send(JSON.stringify({ 
            tipo: 'respuesta', 
            texto: 'Hola, se ha comunicado con el asistente de Serenity Spa. ¿Con quién tengo el gusto de hablar?' 
        }));
        ...
        ws.on('message', async (data) => {
            ...
            try {
                ...
                if (m.tipo === 'transcripcion') {
                    console.log(`🎤 Mensaje: "${m.texto}"`);
                    const r = await procesarComandoVoz(m.texto, cid, ip);
                    console.log(`🤖 Respuesta: "${r.substring(0, 80)}..."`);
                    if (ws.readyState === 1) {
