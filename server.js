...
async function initAllFiles() {
    ...
    if (!fsSync.existsSync(SERVICIOS_FILE)) {
        const serviciosDefault = [
            { id: generarId(), nombre: "Masaje Relajante", precio: "$45", descripcion: "Movimientos suaves para liberar el estrés.", beneficios: ["60 Minutos de relajación", "Aceites esenciales"], efectos: ["Relajación profunda", "Reducción del estrés"], videoUrl: "https://www.youtube.com/embed/9t5LdPZgPPA", imagenWeb: "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?w=800", imagenWhatsApp: "", orden: 1 },
...
// ==================== PREMIUM - GUARDAR USUARIO ====================
app.post('/api/premium/usuario', async (req, res) => {
    const usuario = req.body;
    console.log('📝 Recibiendo usuario premium:', usuario);
    ...
    try {
        ...
        try {
            ...
            console.log(`📂 Archivo premium_usuarios.json leído: ${usuarios.length} usuarios`);
        } catch(e) {
        ...
        if (index >= 0) {
            console.log(`✏️ Actualizando usuario existente: ${usuario.email}`);
        ...
        console.log(`✅ Usuario premium guardado: ${usuario.email}`);
...
app.get('/api/premium/verificar', async (req, res) => {
    try {
        const data = await fs.readFile(PREMIUM_USUARIOS_FILE, 'utf8');
        const usuarios = JSON.parse(data);
        res.json({ total: usuarios.length, usuarios: usuarios });
    } catch(e) {
